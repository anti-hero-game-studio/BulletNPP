...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletMoverSimulationTypes)

UScriptStruct* FBulletMoverSimulationEventData::GetScriptStruct() const
{
	checkf(false, TEXT("%hs is being called erroneously. This must be overridden in derived types!"), __FUNCTION__);
	return FBulletMoverSimulationEventData::StaticStruct();
}

FBulletScheduledInstantMovementEffect FBulletScheduledInstantMovementEffect::ScheduleEffect(UWorld* World, const FBulletMoverTimeStep& TimeStep, TSharedPtr<FBulletInstantMovementEffect> InstantMovementEffect, float SchedulingDelaySeconds)
{
	...
	if (bIsFixedDt)
	{
		...
		return FBulletScheduledInstantMovementEffect(/* ExecutionServerFrame = */ ServerFrame, /* ExecutionServerTime = */ 0.0, /* bIsFixedDt = */ true, InstantMovementEffect);
	}
	else
	{
		...
		return FBulletScheduledInstantMovementEffect(/* ExecutionServerFrame = */ INDEX_NONE, /* ExecutionServerTime = */ ServerTime, /* bIsFixedDt = */ false, InstantMovementEffect);
	}
}

namespace UE::BulletMover
{
	void FBulletSimulationOutputData::Reset()
	{
		SyncState.Reset();
	...
	void FBulletSimulationOutputData::Interpolate(const FBulletSimulationOutputData& From, const FBulletSimulationOutputData& To, float Alpha, double SimTimeMs)
	{
	...
	void FBulletSimulationOutputRecord::FData::Reset()
	{
		TimeStep = FBulletMoverTimeStep();
		SimOutputData.Reset();
	}

	void FBulletSimulationOutputRecord::Add(const FBulletMoverTimeStep& InTimeStep, const FBulletSimulationOutputData& InData)
	{
		...
		if (InTimeStep.bIsResimulating)
		{
			...
			Events.RemoveAllSwap([&InTimeStep](const TSharedPtr<FBulletMoverSimulationEventData> Event) {
				return !Event.IsValid() || (Event->EventTimeMs >= InTimeStep.BaseSimTimeMs);
				});
		}

		// Transfer events to local Events array
		for (TSharedPtr<FBulletMoverSimulationEventData>& Event : Data[CurrentIndex].SimOutputData.Events)
		{
			Events.Add(Event);
		}
	...
	const FBulletSimulationOutputData& FBulletSimulationOutputRecord::GetLatest() const
	{
		return Data[CurrentIndex].SimOutputData;
	}

	void FBulletSimulationOutputRecord::CreateInterpolatedResult(double AtBaseTimeMs, FBulletMoverTimeStep& OutTimeStep, FBulletSimulationOutputData& OutData)
	{
		const uint8 PrevIndex = (CurrentIndex + 1) % 2;
		...
		for (TSharedPtr<FBulletMoverSimulationEventData>& Event : Events)
		{
		...
		Events.RemoveAllSwap([](const TSharedPtr<FBulletMoverSimulationEventData> Event) {
			return !Event.IsValid();
			});
	}

	void FBulletSimulationOutputRecord::Clear()
	{
		CurrentIndex = 1;
		Data[0].Reset();
		Data[1].Reset();
