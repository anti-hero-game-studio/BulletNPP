...
#include "Backends/BulletMoverStandaloneLiaison.h"
#include "Engine/World.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/Controller.h"
#include "BulletMoverComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletMoverStandaloneLiaison)
...
namespace MoverStandaloneBackendCVars
{
	static void OnRunProduceInputOnAnyThreadChanged(IConsoleVariable* Var)
	{
		...
		ForEachObjectOfClass(UBulletMoverStandaloneLiaisonComponent::StaticClass(), [&](UObject* AsObj)
		{
			if (UBulletMoverStandaloneLiaisonComponent* StandaloneLiaison = CastChecked<UBulletMoverStandaloneLiaisonComponent>(AsObj, ECastCheckedType::NullAllowed))
			{
	...
	FAutoConsoleVariableRef CVarMoverStandaloneProduceInputOnAnyThread(
		TEXT("bullet.mover.standalone.RunProduceInputOnAnyThread"),
	...
	FAutoConsoleVariableRef CVarMoverStandaloneRunSimOnAnyThread(
		TEXT("bullet.mover.standalone.RunMovementSimOnAnyThread"),
...
UBulletMoverStandaloneLiaisonComponent::UBulletMoverStandaloneLiaisonComponent(const FObjectInitializer& ObjectInitializer)
...
double UBulletMoverStandaloneLiaisonComponent::GetCurrentSimTimeMs()
{
	return CurrentSimTimeMs;
}

int32 UBulletMoverStandaloneLiaisonComponent::GetCurrentSimFrame()
{
	return CurrentSimFrame;
}


bool UBulletMoverStandaloneLiaisonComponent::ReadPendingSyncState(OUT FBulletMoverSyncState& OutSyncState)
{
	{
...
bool UBulletMoverStandaloneLiaisonComponent::WritePendingSyncState(const FBulletMoverSyncState& SyncStateToWrite)
{
	if (bIsInApplySimulationState)
	{
		UE_LOG(LogBulletMover, Log, TEXT("Attempted to WritePendingSyncState during ApplySimulationState. Skipping. (%s)"), *GetNameSafe(GetOwner()));
...
void UBulletMoverStandaloneLiaisonComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	{
		ensureMsgf(OwnerActor->GetNetMode() == NM_Standalone, TEXT("UBulletMoverStandaloneLiaisonComponent is only valid for use in Standalone projects. Movement will not work properly in networked play."));

		if (UBulletMoverComponent* FoundMoverComp = OwnerActor->FindComponentByClass<UBulletMoverComponent>())
		{
...
FTickFunction* UBulletMoverStandaloneLiaisonComponent::FindTickFunction(EBulletMoverTickPhase MoverTickPhase)
{
	FTickFunction* FoundFunction = nullptr;

	switch (MoverTickPhase)
	{
		case EBulletMoverTickPhase::ProduceInput:
			FoundFunction = &ProduceInputTickFunction;
			break;

		case EBulletMoverTickPhase::SimulateMovement:
			FoundFunction = &SimulateMovementTickFunction;
			break;

		case EBulletMoverTickPhase::ApplyState:
...
void UBulletMoverStandaloneLiaisonComponent::AddTickDependency(UActorComponent* OtherComponent, EBulletMoverTickDependencyOrder TickOrder, EBulletMoverTickPhase TickPhase)
{
	...
	if (OtherComponent && TickFunction)
	{
		switch (TickOrder)
		{
			case EBulletMoverTickDependencyOrder::After:
				...
				break;
			case EBulletMoverTickDependencyOrder::Before:
...
void UBulletMoverStandaloneLiaisonComponent::SetUseAsyncProduceInput(bool bUseAsyncInputProduction)
{
	bUseAsyncProduceInput = bUseAsyncInputProduction;
	ProduceInputTickFunction.bRunOnAnyThread = bUseAsyncProduceInput && MoverStandaloneBackendCVars::RunProduceInputOnAnyThread;
}

bool UBulletMoverStandaloneLiaisonComponent::GetUseAsyncProduceInput() const
{
	return bUseAsyncProduceInput;
}

void UBulletMoverStandaloneLiaisonComponent::SetEnableProduceInput(bool bEnableInputProduction)
{
...
bool UBulletMoverStandaloneLiaisonComponent::GetEnableProduceInput() const
{
	return ProduceInputTickFunction.IsTickFunctionEnabled();
}

void UBulletMoverStandaloneLiaisonComponent::SetUseAsyncMovementSimulationTick(bool bUseAsyncMovementSim)
{
	bUseAsyncMovementSimulationTick = bUseAsyncMovementSim;
	SimulateMovementTickFunction.bRunOnAnyThread = bUseAsyncMovementSimulationTick && MoverStandaloneBackendCVars::RunMovementSimOnAnyThread;
}

bool UBulletMoverStandaloneLiaisonComponent::GetUseAsyncMovementSimulationTick() const
{
	return bUseAsyncMovementSimulationTick;
}


void UBulletMoverStandaloneLiaisonComponent::RegisterComponentTickFunctions(bool bRegister)
{
	...
	if (bRegister)
	{
		if (SetupActorComponentTickFunction(&ProduceInputTickFunction))
		{
			...
			if (APawn* OwnerPawn = Cast<APawn>(GetOwner()))
			{
				...
				OwnerPawn->ReceiveControllerChangedDelegate.AddDynamic(this, &UBulletMoverStandaloneLiaisonComponent::OnControllerChanged);
			}
	...
	else
	{
		if (APawn* OwnerPawn = Cast<APawn>(GetOwner()))
		{
			...
			OwnerPawn->ReceiveControllerChangedDelegate.RemoveDynamic(this, &UBulletMoverStandaloneLiaisonComponent::OnControllerChanged);
		}
...
void UBulletMoverStandaloneLiaisonComponent::UpdateSimulationTime()
{
	CurrentSimTimeMs = GetWorld()->GetTimeSeconds() * 1000.0;
	CurrentSimFrame = GFrameCounter;
}

void UBulletMoverStandaloneLiaisonComponent::TickInputProduction(float DeltaSeconds)
{
	UpdateSimulationTime();
...
void UBulletMoverStandaloneLiaisonComponent::TickMovementSimulation(float DeltaSeconds)
{
...
void UBulletMoverStandaloneLiaisonComponent::TickApplySimulationState(float DeltaSeconds)
{
...
void UBulletMoverStandaloneLiaisonComponent::OnControllerChanged(APawn* Pawn, AController* OldController, AController* NewController)
{
	if (OldController)
	{
...
// FBulletMoverStandaloneProduceInputTickFunction ---------------------

void FBulletMoverStandaloneProduceInputTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	// ExecuteTickHelper does the null check
	UBulletMoverStandaloneLiaisonComponent* TargetComp = Target.Get();
...
FString FBulletMoverStandaloneProduceInputTickFunction::DiagnosticMessage()
{
	return Target->GetFullName() + TEXT("[UBulletMoverStandaloneLiaisonComponent::ProduceInputTick]");
}

FName FBulletMoverStandaloneProduceInputTickFunction::DiagnosticContext(bool bDetailed)
{
	if (bDetailed)
	{
		return FName(*FString::Printf(TEXT("BulletMoverStandaloneProduceInputTickFunction/%s"), *GetFullNameSafe(Target.Get())));
	}

	static const FName MoverStandaloneProduceInputTickFunctionFName(TEXT("BulletMoverStandaloneProduceInputTickFunction"));
	return MoverStandaloneProduceInputTickFunctionFName;
}

// FBulletMoverStandaloneSimulateMovementTickFunction ---------------------

void FBulletMoverStandaloneSimulateMovementTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	UBulletMoverStandaloneLiaisonComponent* TargetComp = Target.Get();
	FActorComponentTickFunction::ExecuteTickHelper(TargetComp, /*bTickInEditor=*/ false, DeltaTime, TickType, [TargetComp](float DilatedTime)
		{
...
FString FBulletMoverStandaloneSimulateMovementTickFunction::DiagnosticMessage()
{
	return Target->GetFullName() + TEXT("[UBulletMoverStandaloneLiaisonComponent::SimulateMovement]");
}

FName FBulletMoverStandaloneSimulateMovementTickFunction::DiagnosticContext(bool bDetailed)
{
	if (bDetailed)
	{
		return FName(*FString::Printf(TEXT("BulletMoverStandaloneSimulateMovementTickFunction/%s"), *GetFullNameSafe(Target.Get())));
	}

	static const FName MoverStandaloneSimulateMovementTickFunctionFName(TEXT("BulletMoverStandaloneSimulateMovementTickFunction"));
	return MoverStandaloneSimulateMovementTickFunctionFName;
}


// FBulletMoverStandaloneApplyStateTickFunction ---------------------

void FBulletMoverStandaloneApplyStateTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	UBulletMoverStandaloneLiaisonComponent* TargetComp = Target.Get();
	FActorComponentTickFunction::ExecuteTickHelper(TargetComp, /*bTickInEditor=*/ false, DeltaTime, TickType, [TargetComp](float DilatedTime)
		{
...
FString FBulletMoverStandaloneApplyStateTickFunction::DiagnosticMessage()
{
	return Target->GetFullName() + TEXT("[UBulletMoverStandaloneLiaisonComponent::ApplyState]");
}

FName FBulletMoverStandaloneApplyStateTickFunction::DiagnosticContext(bool bDetailed)
{
	if (bDetailed)
	{
		return FName(*FString::Printf(TEXT("BulletMoverStandaloneApplyStateTickFunction/%s"), *GetFullNameSafe(Target.Get())));
	}

	static const FName MoverStandaloneApplyStateTickFunctionFName(TEXT("BulletMoverStandaloneApplyStateTickFunction"));
