...
#include "GameplayDebuggerCategory_BulletMover.h"
...
namespace
{
	FAutoConsoleVariableRef CVars_GameplayDebuggerCategory_BulletMover[] = 
	{
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.MaxMoveIntentDrawLength"), 
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.OrientationDrawLength"),
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.ShowTrajectory"), 
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.ShowTrail"),
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.ShowCorrections"),
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.ShowStateArrows"),
		...
		FAutoConsoleVariableRef(TEXT("bullet.mover.debug.ShowInputArrows"),
...
FGameplayDebuggerCategory_BulletMover::FGameplayDebuggerCategory_BulletMover()
{
	SetDataPackReplication<FRepData>(&DataPack);
}


void FGameplayDebuggerCategory_BulletMover::CollectData(APlayerController* OwnerPC, AActor* DebugActor)
{
	APawn* MyPawn = Cast<APawn>(DebugActor);
	UBulletMoverComponent* MyMoverComponent = MyPawn ? Cast<UBulletMoverComponent>(MyPawn->GetComponentByClass(UBulletMoverComponent::StaticClass())) : nullptr;

	if (MyPawn)
	{
		// Get the debug component 
		UBulletMoverDebugComponent* MoverDebugComponent = Cast<UBulletMoverDebugComponent>(MyPawn->GetComponentByClass(UBulletMoverDebugComponent::StaticClass()));
		// Make it if we can't find it
		if (!MoverDebugComponent)
		{
			MoverDebugComponent = Cast<UBulletMoverDebugComponent>(MyPawn->AddComponentByClass(UBulletMoverDebugComponent::StaticClass(), false, FTransform::Identity, false));
	...
	if (MyMoverComponent)
	{
		...
		for (const TPair<FName, TObjectPtr<UBulletBaseMovementMode>>& ModeIter : MyMoverComponent->MovementModes)
		{
			UBulletBaseMovementMode* MappedMode = ModeIter.Value;
			...
			{
				for (UBulletBaseMovementModeTransition* Transition : MyMoverComponent->MovementModes[ModeIter.Key]->Transitions)
				{
		...
		for (UBulletBaseMovementModeTransition* Transition : MyMoverComponent->Transitions)
		{
			DataPack.ActiveTransitions.Add(FString::Printf(TEXT("%s (global)"), (Transition ? *Transition->GetClass()->GetName() : TEXT("null"))));
		}
		
		const FBulletMoverSyncState& SyncState = MyMoverComponent->GetSyncState();

		for (const TSharedPtr<FBulletLayeredMoveBase>& ActiveMove : SyncState.LayeredMoves.GetActiveMoves())
		{
...
void FGameplayDebuggerCategory_BulletMover::DrawData(APlayerController* OwnerPC, FGameplayDebuggerCanvasContext& CanvasContext)
{
...
TSharedRef<FGameplayDebuggerCategory> FGameplayDebuggerCategory_BulletMover::MakeInstance()
{
	return MakeShareable(new FGameplayDebuggerCategory_BulletMover());
}


void FGameplayDebuggerCategory_BulletMover::DrawOverheadInfo(AActor& DebugActor, FGameplayDebuggerCanvasContext& CanvasContext)
{
...
void FGameplayDebuggerCategory_BulletMover::DrawInWorldInfo(AActor& DebugActor, FGameplayDebuggerCanvasContext& CanvasContext)
{
	UWorld* MyWorld = CanvasContext.GetWorld();

	UBulletMoverComponent* MoverComp = Cast<UBulletMoverComponent>(DebugActor.GetComponentByClass(UBulletMoverComponent::StaticClass()));
...
void FGameplayDebuggerCategory_BulletMover::FRepData::Serialize(FArchive& Ar)
{
	Ar << PawnName;
	Ar << LocalRole;
	Ar << MovementModeName;
