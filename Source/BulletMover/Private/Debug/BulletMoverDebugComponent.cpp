...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletMoverDebugComponent)

UBulletMoverDebugComponent::UBulletMoverDebugComponent()
{
...
void UBulletMoverDebugComponent::InitializeComponent()
{
...
void UBulletMoverDebugComponent::BeginPlay()
{
	Super::BeginPlay();
	
	if (UBulletMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UBulletMoverComponent>())
	{
		MoverComp->OnPostSimulationTick.AddDynamic(this, &UBulletMoverDebugComponent::OnMovementSimTick);
	}
}

void UBulletMoverDebugComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (UBulletMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UBulletMoverComponent>())
	{
...
void UBulletMoverDebugComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (bIsTrackingHistory)
	{
		UBulletMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UBulletMoverComponent>();
...
void UBulletMoverDebugComponent::SetHistoryTracking(float SecondsToTrack, float SamplesPerSecond)
{
...
TArray<FBulletTrajectorySampleInfo> UBulletMoverDebugComponent::GetPastTrajectory() const
{
	if (bIsTrackingHistory && !HistorySamples.IsEmpty())
	{
		TArray<FBulletTrajectorySampleInfo> HistoryAsArray;
	...
	return TArray<FBulletTrajectorySampleInfo>();
}

void UBulletMoverDebugComponent::InitHistoryTracking()
{
	if (UBulletMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UBulletMoverComponent>())
	{
		...
		if (bIsTrackingHistory)
		{
			...
			MoverComp->OnPostSimulationRollback.RemoveAll(this);
			MoverComp->OnPostSimulationRollback.AddDynamic(this, &UBulletMoverDebugComponent::OnHistoryTrackingRollback);
		}
		else
		{
		...
		MoverComp->OnPostSimulationRollback.AddDynamic(this, &UBulletMoverDebugComponent::OnMovementSimRollback);
	}
}

void UBulletMoverDebugComponent::UpdateHistoryTrackingForFrame(const FBulletMoverTimeStep& TimeStep, const FBulletMoverSyncState* SyncState, const FBulletMoverAuxStateContext* AuxState)
{
	...
	HistorySamples.RemoveAll([&](const FBulletTrajectorySampleInfo& Sample)
		{
	...
	FBulletTrajectorySampleInfo* MostRecentSample = HistorySamples.IsEmpty() ? nullptr : &HistorySamples.Last();
	...
	if (!MostRecentSample || (CurrentSimTimeMs > MostRecentSample->SimTimeMs + MaxTimeBetweenSamplesMs))
	{
		...
		FBulletTrajectorySampleInfo Sample;
...
void UBulletMoverDebugComponent::OnHistoryTrackingRollback(const FBulletMoverTimeStep& NewTimeStep, const FBulletMoverTimeStep& InvalidatedTimeStep)
{
...
void UBulletMoverDebugComponent::DrawTrajectory()
{
	...
	if (UBulletMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UBulletMoverComponent>())
	{
		...
		TArray<FBulletTrajectorySampleInfo> TrajectorySamples = MoverComp->GetPredictedTrajectory(PredictionParams);

		for (int32 i = 0; i < TrajectorySamples.Num() - 1; ++i)
		{
			const FBulletTrajectorySampleInfo& Sample = TrajectorySamples[i];
			const FBulletTrajectorySampleInfo& NextSample = TrajectorySamples[i + 1];
	...
	TArray<FBulletTrajectorySampleInfo> PastTrajectorySamples = GetPastTrajectory(); 

	for (int i = 0; i < PastTrajectorySamples.Num()-1; ++i)
	{
		const FBulletTrajectorySampleInfo& Sample = PastTrajectorySamples[i];
		const FBulletTrajectorySampleInfo& NextSample = PastTrajectorySamples[i + 1];
...
void UBulletMoverDebugComponent::OnMovementSimTick(const FBulletMoverTimeStep& TimeStep)
{
...
void UBulletMoverDebugComponent::OnMovementSimRollback(const FBulletMoverTimeStep& NewTimeStep, const FBulletMoverTimeStep& InvalidatedTimeStep)
{
...
void UBulletMoverDebugComponent::DrawCorrections()
{
	if (ClientLocations.Num() != CorrectedLocations.Num())
	{
		UE_LOG(LogBulletMover, Warning, TEXT("Correction arrays differed in size!"));
...
void UBulletMoverDebugComponent::DrawTrail()
{
	const float HalfHeight = GetOwner()->GetSimpleCollisionHalfHeight();
