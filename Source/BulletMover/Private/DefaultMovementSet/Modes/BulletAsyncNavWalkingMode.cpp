...
#include "DefaultMovementSet/Modes/BulletAsyncNavWalkingMode.h"
...
#include "MoveLibrary/BulletNavMovementUtils.h"
#include "NavMesh/RecastNavMesh.h"
#include "VisualLogger/VisualLogger.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletAsyncNavWalkingMode)

#if ENABLE_VISUAL_LOG
static const FName AsyncWalkModeLogCategory = FName("AsyncNavWalkingMode");
#endif

namespace BulletAsyncWalkingModeCVars
{
	float OverrideRaycastInterval = -1.0f;
	FAutoConsoleVariableRef CVarOverrideNavProjectionInterval(TEXT("BulletMover.AsyncNav.OverrideRaycastInterval"), OverrideRaycastInterval, TEXT(""));

	bool bUseNavMeshNormal = false;
	FAutoConsoleVariableRef CVarUseNavMeshNormal(TEXT("BulletMover.AsyncNav.UseNavMeshNormal"), bUseNavMeshNormal, TEXT(""));
}

static const FName MoveWithoutNavMeshSubstepName = "MoveWithoutNavMesh";

UBulletAsyncNavWalkingMode::UBulletAsyncNavWalkingMode()
...
void UBulletAsyncNavWalkingMode::GenerateMove_Implementation(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, FBulletProposedMove& OutProposedMove) const
{
...
void UBulletAsyncNavWalkingMode::SimulationTick_Implementation(const FBulletSimulationTickParams& Params, FBulletMoverTickEndData& OutputState)
{
	SCOPED_NAMED_EVENT(BulletAsyncNavWalkingMode_SimulationTick, FColor::Yellow);
	...
	if (CachedNavLocation.HasNodeRef())
	{
		...
		{
			const float DistSq = UBulletMovementUtils::ProjectToGravityFloor(StartingFeetLocation - CachedNavLocation.Location, UpDirection).SizeSquared();
	...
	else
	{
		...
		{
			UBulletMovementUtils::SetGravityVerticalComponent(TargetFeetLocation, CachedNavLocation.Location.Dot(UpDirection), UpDirection);
		}
		...
		if (NavDataInterface.IsValid())
		{
			...
			if (bSlideAlongNavMeshEdge && !bIsOnNavLink)
			{
				...
				if (bHasValidCachedNavLocation)
				{
					...
					{
						TargetFeetLocation = UBulletMovementUtils::ProjectToGravityFloor(DestNavLocation.Location, UpDirection) + UBulletMovementUtils::GetGravityVerticalComponent(TargetFeetLocation, UpDirection);
					}
				}
			}
			else
		...
		if (!bFoundPointOnNavMesh)
		{
			...
			switch (BehaviorOffNavMesh)
			{
				...
				case EBulletOffNavMeshBehavior::SwitchToWalking:
					UE_LOG(LogBulletMover, Verbose, TEXT("%s could not find valid navigation data at location %s. Switching to walking mode."), *GetNameSafe(MoverComp->GetOwner()), *TargetFeetLocation.ToCompactString());
				...
				case EBulletOffNavMeshBehavior::MoveWithoutNavMesh:
					...
					MoveRecord.Append(FBulletMovementSubstep(MoveWithoutNavMeshSubstepName, ProposedMove.LinearVelocity * DeltaSeconds, true));
					break;

				case EBulletOffNavMeshBehavior::DoNotMove:
					UE_LOG(LogBulletMover, Verbose, TEXT("%s could not find valid navigation data at location %s. Cannot move."), *GetNameSafe(MoverComp->GetOwner()), *TargetFeetLocation.ToCompactString());
					// nothing to be done
					break;
	...
	{
		FVector NewFeetLocation = UBulletMovementUtils::ProjectToGravityFloor(TargetFeetLocation, UpDirection) + UBulletMovementUtils::GetGravityVerticalComponent(DestNavLocation.Location, UpDirection);
		if (bProjectNavMeshWalking)
		{
			const float TotalCapsuleHeight = SimpleHalfHeight * 2.0f;
		...
		if (!AdjustedDelta.IsNearlyZero())
		{
			...
			FBulletMoverCollisionParams CollisionParams(Params.MovingComps.UpdatedComponent.Get());
			...
			if (UBulletAsyncMovementUtils::TestDepenetratingMove(Params.MovingComps, StartLocation, StartLocation + AdjustedDelta, StartRotation, TargetRotation, bSweepWhileNavWalking, CollisionParams, OUT MoveHitResult, IN OUT MoveRecord))
			{
...
bool UBulletAsyncNavWalkingMode::FindNavFloor(const FVector& TestLocation, FNavLocation& OutNavFloorLocation, const INavigationDataInterface* NavData) const
{
...
UObject* UBulletAsyncNavWalkingMode::GetTurnGenerator()
{
	return TurnGenerator;
}

void UBulletAsyncNavWalkingMode::SetTurnGeneratorClass(TSubclassOf<UObject> TurnGeneratorClass)
{
...
void UBulletAsyncNavWalkingMode::Activate()
{
	Super::Activate();
	
	if (UBulletMoverComponent* MoverComp = GetMoverComponent())
	{
		if (UBulletMoverBlackboard* SimBlackboard = MoverComp->GetSimBlackboard_Mutable())
		{
...
const INavigationDataInterface* UBulletAsyncNavWalkingMode::GetNavData() const
{
...
void UBulletAsyncNavWalkingMode::FindBestNavMeshLocation(const FVector& TraceStart, const FVector& TraceEnd, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, FHitResult& OutHitResult) const
{
	...
	if (BulletAsyncWalkingModeCVars::bUseNavMeshNormal && CachedNavLocation.HasNodeRef() && MultiTraceHits.Num() > 0)
	{
...
FVector UBulletAsyncNavWalkingMode::ProjectLocationFromNavMesh(float DeltaSeconds, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, float UpOffset, float DownOffset)
{
	...
	if (NavMeshProjectionTimer <= 0.0f || !bHasValidFloorResult)
	{
		...
		float ModTime = 0.f;
		float Interval = BulletAsyncWalkingModeCVars::OverrideRaycastInterval >= 0.0f ? BulletAsyncWalkingModeCVars::OverrideRaycastInterval : NavMeshProjectionInterval;
...
void UBulletAsyncNavWalkingMode::OnRegistered(const FName ModeName)
{
...
void UBulletAsyncNavWalkingMode::OnUnregistered()
{
