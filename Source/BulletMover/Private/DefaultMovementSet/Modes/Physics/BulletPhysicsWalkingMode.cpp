...
void UBulletPhysicsWalkingMode::GenerateMove_Implementation(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, FBulletProposedMove& OutProposedMove) const
{
	...
	{
		IntendedOrientation_WorldSpace = UE::BulletNetQuant::QuantizeRotatorCompressedShort(CharacterInputs->GetOrientationIntentDir_WorldSpace().ToOrientationRotator());
	}
...
UObject* UBulletPhysicsWalkingMode::GetTurnGenerator()
{
	return TurnGenerator;
}

void UBulletPhysicsWalkingMode::SetTurnGeneratorClass(TSubclassOf<UObject> TurnGeneratorClass)
{
...
void UBulletPhysicsWalkingMode::CaptureFinalState(const FVector FinalLocation, const FRotator FinalRotation, bool bDidAttemptMovement, const FBulletFloorCheckResult& FloorResult, const FBulletMovementRecord& Record, const FVector& AngularVelocityDegrees, FBulletUpdatedMotionState& OutputSyncState) const
{
...
FBulletRelativeBaseInfo UBulletPhysicsWalkingMode::UpdateFloorAndBaseInfo(const FBulletFloorCheckResult& FloorResult) const
{
