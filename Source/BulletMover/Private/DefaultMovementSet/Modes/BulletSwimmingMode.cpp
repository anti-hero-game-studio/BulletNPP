...
void USwimmingMode::GenerateMove_Implementation(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, FBulletProposedMove& OutProposedMove) const
{
	...
	FBulletWaterCheckResult LastWaterResult;
	...
	// Buoyancy Bobbing
	{
		const FBulletWaterFlowSplineData& WaterData = LastWaterResult.WaterSplineData;
	...
	// Calculate and apply the requested move here
	{
		...
		// Calculate the move
		OutProposedMove = UBulletWaterMovementUtils::ComputeControlledWaterMove(Params);
...
void USwimmingMode::OnRegistered(const FName ModeName)
{
	...
	{
		CommonLegacySettings = MoverComp->FindSharedSettings<UBulletCommonLegacyMovementSettings>();
...
bool USwimmingMode::AttemptJump(const FBulletSimulationTickParams& Params, float UpwardsSpeed, FBulletMoverTickEndData& OutputState)
{
	...
	// TODO: Use UBulletMoverSimulation to queue instant movement effects when it is passed as part of FBulletSimulationTickParams
