...
#include "DefaultMovementSet/LayeredMoves/BulletRootMotionAttributeLayeredMove.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletRootMotionAttributeLayeredMove)

UE_DEFINE_GAMEPLAY_TAG_COMMENT(BulletMover_AnimRootMotion_MeshAttribute, "BulletMover.AnimRootMotion.MeshAttribute", "Signifies an association with root motion that comes via a skeletal mesh attribute");


#if !UE_BUILD_SHIPPING
FAutoConsoleVariable CVarLogRootMotionAttrSteps(
	TEXT("bullet.mover.debug.LogRootMotionAttrSteps"),
	false,
...
FAutoConsoleVariable CVarDisableRootMotionAttrContributions(
	TEXT("bullet.mover.debug.DisableRootMotionAttributes"),
	false,
...
FAutoConsoleVariableRef CVarExcessiveLinearSpeedSquaredThreshold(
	TEXT("bullet.mover.debug.RootMotionAttributesExcessiveSpeedSq"),
...
FBulletLayeredMove_RootMotionAttribute::FBulletLayeredMove_RootMotionAttribute()
{
	DurationMs = -1.f;
	MixMode = EBulletMoveMixMode::OverrideAll;
}

bool FBulletLayeredMove_RootMotionAttribute::GenerateMove(const FBulletMoverTickStartData& SimState, const FBulletMoverTimeStep& TimeStep, const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, FBulletProposedMove& OutProposedMove)
{
#if !UE_BUILD_SHIPPING
	...
	if (bDidAttrHaveRootMotion)
	{
		...
		if (MoverComp->HasGameplayTagInState(SimState.SyncState, BulletMover_SkipVerticalAnimRootMotion, false) && MixMode == EBulletMoveMixMode::OverrideAll)
		{
			OutProposedMove.MixMode = EBulletMoveMixMode::OverrideAllExceptVerticalVelocity;
		}
...
#if !UE_BUILD_SHIPPING
		UE_CLOG(CVarLogRootMotionAttrSteps->GetBool(), LogBulletMover, Log, TEXT("RootMotionAttr. SimF %i (dt %.3f) => LocalT: %s (WST: %s)  XY Speed: %.6f Z: %.6f   AngV: %s"),
		...
		{
			UE_LOG(LogBulletMover, Warning, TEXT("RootMotionAttr on %s has excessive speed.  LocalTrans: %s (DT: %.5f) -> XY Vel: %.6f  Z Vel: %.6f"),
...
bool FBulletLayeredMove_RootMotionAttribute::HasGameplayTag(FGameplayTag TagToFind, bool bExactMatch) const
{
	static const FGameplayTag MeshAttributeTag = BulletMover_AnimRootMotion_MeshAttribute.GetTag();
...
FBulletLayeredMoveBase* FBulletLayeredMove_RootMotionAttribute::Clone() const
{
	FBulletLayeredMove_RootMotionAttribute* CopyPtr = new FBulletLayeredMove_RootMotionAttribute(*this);
	return CopyPtr;
}

void FBulletLayeredMove_RootMotionAttribute::NetSerialize(FArchive& Ar)
{
	Super::NetSerialize(Ar);
}

UScriptStruct* FBulletLayeredMove_RootMotionAttribute::GetScriptStruct() const
{
	return FBulletLayeredMove_RootMotionAttribute::StaticStruct();
}

FString FBulletLayeredMove_RootMotionAttribute::ToSimpleString() const
{
	return FString::Printf(TEXT("RootMotionAttribute"));
}

void FBulletLayeredMove_RootMotionAttribute::AddReferencedObjects(class FReferenceCollector& Collector)
{
