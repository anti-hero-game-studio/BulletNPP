...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletGroundMovementUtils)

FBulletProposedMove UBulletGroundMovementUtils::ComputeControlledGroundMove(const FBulletGroundMoveParams& InParams)
{
	FBulletProposedMove OutMove;

	const FVector MoveDirIntent = UBulletMovementUtils::ComputeDirectionIntent(InParams.MoveInput, InParams.MoveInputType, InParams.MaxSpeed);

	const FPlane MovementPlane(FVector::ZeroVector, InParams.UpDirection);
	FVector MoveDirIntentInMovementPlane = UBulletMovementUtils::ConstrainToPlane(MoveDirIntent, MovementPlane, true);

	const FPlane GroundSurfacePlane(FVector::ZeroVector, InParams.GroundNormal);
	OutMove.DirectionIntent = UBulletMovementUtils::ConstrainToPlane(MoveDirIntentInMovementPlane, GroundSurfacePlane, true);
	...
	const FVector Velocity = UBulletMovementUtils::ComputeVelocity(ComputeVelocityParams);
	OutMove.LinearVelocity = UBulletMovementUtils::ConstrainToPlane(Velocity, GroundSurfacePlane, true);
...
bool UBulletGroundMovementUtils::TryMoveToStepUp(const FBulletMovingComponentSet& MovingComps, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, float FloorSweepDistance, const FVector& MoveDelta, const FHitResult& MoveHitResult, const FBulletFloorCheckResult& CurrentFloor, bool bIsFalling, FBulletOptionalFloorCheckResult* OutFloorTestResult, FBulletMovementRecord& MoveRecord)
{
	...
	TArray<FBulletMovementSubstep> QueuedSubsteps;	// keeping track of substeps before committing, because some moves can be backed out
	...
	{
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("Not stepping up due to top of capsule hitting something"));
		return false;
	}
	...
	if (GravDir.IsZero())
	{
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("Not stepping up because there's no gravity"));
		return false;
	}
	...
	if (CurrentFloor.IsWalkableFloor())
	{
		...
		const bool bHitVerticalFace = !UBulletFloorQueryUtils::IsWithinEdgeTolerance(MoveHitResult.Location, MoveHitResult.ImpactPoint, PawnRadius, UpDirection);
	...
	{
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("Not stepping up because the impact is below us"));
		return false;
	}
	...
	const bool bDidStepUp = UBulletMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, UpAdjustment, PawnRotation, true, MOVECOMP_NoFlags, &SweepUpHit, ETeleportType::None);

	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToStepUp Up: %s (role %i) UpAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *UpAdjustment.ToCompactString(), bDidStepUp);

	if (SweepUpHit.bStartPenetrating)
	{
		// Undo movement
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reverting step-up attempt because we started in a penetrating state"));
		ScopedStepUpMovement.RevertMove();
		return false;
	}

	// Cache upwards substep
	QueuedSubsteps.Add(FBulletMovementSubstep(StepUpSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, false));
	LastComponentLocation = CapsuleComponent->GetComponentLocation();

	// step fwd
	FHitResult StepFwdHit(1.f);
	const bool bDidStepFwd = UBulletMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, MoveDelta, PawnRotation, true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToStepUp Fwd: %s (role %i) MoveDelta=%s DidMove=%i"),
	...
	if (StepFwdHit.bBlockingHit)
	{
		...
		{
			// Undo movement
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we started in a penetrating state"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}

		UBulletMoverComponent* MoverComp = MovingComps.MoverComponent.Get();
		// If we hit something above us and also something ahead of us, we should notify about the upward hit as well.
		// The forward hit will be handled later (in the bSteppedOver case below).
		// In the case of hitting something above but not forward, we are not blocked from moving so we don't need the notification.
		if (MoverComp && SweepUpHit.bBlockingHit && StepFwdHit.bBlockingHit)
		{
			FBulletMoverOnImpactParams ImpactParams(NAME_None, SweepUpHit, MoveDelta);
		...
		if (MoverComp)
		{
			FBulletMoverOnImpactParams ImpactParams(NAME_None, StepFwdHit, MoveDelta);
			MoverComp->HandleImpact(ImpactParams);
		}
		
		if (bIsFalling)
		{
			QueuedSubsteps.Add( FBulletMovementSubstep(StepFwdSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, true) );

			// Commit queued substeps to movement record
			for (FBulletMovementSubstep Substep : QueuedSubsteps)
			{
		...
		QueuedSubsteps.Add( FBulletMovementSubstep(SlideSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, true) );
		LastComponentLocation = CapsuleComponent->GetComponentLocation();
		MoveRecord.UnlockRelevancy();

		if (bIsFalling)
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we could not adjust without falling"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because no movement differences occurred"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
	}
	...
	const bool bDidStepDown = UBulletMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, StepDownAdjustment, CapsuleComponent->GetComponentQuat(), true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToStepUp Down: %s (role %i) StepDownAdjustment=%s DidMove=%i"),
	...
	if (StepFwdHit.bStartPenetrating)
	{
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reverting step-down attempt during step-up/step-fwd, because we started in a penetrating state"));
		ScopedStepUpMovement.RevertMove();
		return false;
	}

	FBulletOptionalFloorCheckResult StepDownResult;
	if (StepFwdHit.IsValidBlockingHit())
	{
		...
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, because it made us travel too high (too high Height %.3f) up from floor base %f to %f"), DeltaDot, PawnInitialFloorBaseDot, StepFwdHit.ImpactPoint.Z);
			ScopedStepUpMovement.RevertMove();
			return false;
		}

		// Reject unwalkable surface normals here.
		if (!UBulletFloorQueryUtils::IsHitSurfaceWalkable(StepFwdHit, UpDirection, MaxWalkSlopeCosine))
		{
			// Reject if normal opposes movement direction
			const bool bNormalTowardsMe = (MoveDelta | StepFwdHit.ImpactNormal) < 0.f;
			if (bNormalTowardsMe)
			{
				UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s opposed to movement"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
			...
			{
				UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s above old position)"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		}
		...
		if (!UBulletFloorQueryUtils::IsWithinEdgeTolerance(StepFwdHit.Location, StepFwdHit.ImpactPoint, PawnRadius, UpDirection))
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being outside edge tolerance"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being up onto surface with !CanStepUpOnHitSurface")); 
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (OutFloorTestResult != NULL)
		{
			...
			UBulletFloorQueryUtils::FindFloor(MovingComps, FloorSweepDistance, MaxWalkSlopeCosine, bUseFlatBaseForFloorChecks, CapsuleComponent->GetComponentLocation(), StepDownResult.FloorTestResult);
			...
			if (StepFwdHit.Location.Dot(UpDirection) > OldLocationDot)
			{
				...
				{
					UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to it being an unperchable step")); 
					ScopedStepUpMovement.RevertMove();
					return false;
				}
			}
	...
	QueuedSubsteps.Add(FBulletMovementSubstep(StepDownSubstepName, CapsuleComponent->GetComponentLocation() - LastComponentLocation, false));
	LastComponentLocation = CapsuleComponent->GetComponentLocation();
...
static bool TryMoveToAdjustHeightAboveFloorInternal(const FBulletMovingComponentSet& MovingComps, FBulletFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, bool bPushAwayOnly, FBulletMovementRecord& MoveRecord)
{
	...
	if (OldFloorDist < UE::FloorQueryUtility::MIN_FLOOR_DIST || OldFloorDist > UE::FloorQueryUtility::MAX_FLOOR_DIST)
	{
		...
		UBulletMovementUtils::TrySafeMoveUpdatedComponent(MovingComps,
		...
		else
		{
			...
			if (UBulletFloorQueryUtils::IsHitSurfaceWalkable(AdjustHit, UpDirection, MaxWalkSlopeCosine))
			{
...
bool UBulletGroundMovementUtils::TryMoveToAdjustHeightAboveFloor(const FBulletMovingComponentSet& MovingComps, FBulletFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FBulletMovementRecord& MoveRecord)
{
...
bool UBulletGroundMovementUtils::TryMoveToKeepMinHeightAboveFloor(const FBulletMovingComponentSet& MovingComps, FBulletFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FBulletMovementRecord& MoveRecord)
{
	return TryMoveToAdjustHeightAboveFloorInternal(MovingComps, CurrentFloor, MaxWalkSlopeCosine, /*bPushAwayOnly*/true, MoveRecord);
}


float UBulletGroundMovementUtils::TryWalkToSlideAlongSurface(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, FHitResult& Hit, bool bHandleImpact, FBulletMovementRecord& MoveRecord, float MaxWalkSlopeCosine, float MaxStepHeight)
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
	...
	if (UBulletMoverComponent* MoverComponent = MovingComps.MoverComponent.Get())
	{
		...
		if (NormalDot > 0.f && !UBulletFloorQueryUtils::IsHitSurfaceWalkable(Hit, UpDirection, MaxWalkSlopeCosine))
		{
		...
		FVector SlideDelta = UBulletMovementUtils::ComputeSlideDelta(MovingComps, Delta, PctOfDeltaToMove, SafeWalkNormal, Hit);
		...
		if (SlideDelta.Dot(Delta) > 0.f)
		{
			...
			if (Hit.IsValidBlockingHit())
			{
				...
				SlideDelta = UBulletMovementUtils::ComputeTwoWallAdjustedDelta(MovingComps, SlideDelta, Hit, OldSafeHitNormal);
				const float SlideDeltaDot = SlideDelta.Dot(UpDirection);
				const float HitNormalDot = Hit.Normal.Dot(UpDirection);
				if (SlideDeltaDot > 0.f && UBulletFloorQueryUtils::IsHitSurfaceWalkable(Hit, UpDirection, MaxWalkSlopeCosine) && HitNormalDot > UE_KINDA_SMALL_NUMBER)
				{
...
FVector UBulletGroundMovementUtils::ComputeDeflectedMoveOntoRamp(const FVector& OrigMoveDelta, const FVector& UpDirection, const FHitResult& RampHitResult, float MaxWalkSlopeCosine, const bool bHitFromLineTrace)
{
	...
	if (FloorNormalDot < (1.f - UE_KINDA_SMALL_NUMBER) && FloorNormalDot > UE_KINDA_SMALL_NUMBER && 
		...
		UBulletFloorQueryUtils::IsHitSurfaceWalkable(RampHitResult, UpDirection, MaxWalkSlopeCosine) && !bHitFromLineTrace)
	{
		// Compute a vector that moves parallel to the surface, by projecting the horizontal movement direction onto the ramp.
		const FPlane RampSurfacePlane(FVector::ZeroVector, FloorNormal);
		return UBulletMovementUtils::ConstrainToPlane(OrigMoveDelta, RampSurfacePlane, true);
	}

	return OrigMoveDelta;
}

bool UBulletGroundMovementUtils::CanStepUpOnHitSurface(const FHitResult& Hit)
{
	if (!Hit.IsValidBlockingHit())
	{
		return false;
...
float UBulletGroundMovementUtils::TestGroundedMoveAlongHitSurface(const FBulletMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, bool bHandleImpact, float MaxStepHeight, float MaxWalkSlopeCosine, FHitResult& InOutHit, FBulletMovementRecord& InOutMoveRecord)
{
	...
	if (UBulletMoverComponent* MoverComponent = MovingComps.MoverComponent.Get())
	{
		...
		// We don't want to be pushed up an unwalkable surface.
		if (NormalDot > 0.f && !UBulletFloorQueryUtils::IsHitSurfaceWalkable(InOutHit, UpDirection, MaxWalkSlopeCosine))
		{
			SafeWalkNormal = FVector::VectorPlaneProject(SafeWalkNormal, -UpDirection).GetSafeNormal();
		}
		...
		FVector SlideDelta = UBulletMovementUtils::ComputeSlideDelta(MovingComps, OriginalMoveDelta, PctOfOrigDeltaToSlide, SafeWalkNormal, InOutHit);
		...
		if (SlideDelta.Dot(OriginalMoveDelta) > 0.f)
		{
			...
			if (InOutHit.IsValidBlockingHit())
			{
				...
				// Compute new slide normal when hitting multiple surfaces.
				SlideDelta = UBulletMovementUtils::ComputeTwoWallAdjustedDelta(MovingComps, SlideDelta, InOutHit, OldSafeHitNormal);
				const float SlideDeltaDot = SlideDelta.Dot(UpDirection);
				const float HitNormalDot = InOutHit.Normal.Dot(UpDirection);
				if (SlideDeltaDot > 0.f && UBulletFloorQueryUtils::IsHitSurfaceWalkable(InOutHit, UpDirection, MaxWalkSlopeCosine) && HitNormalDot > UE_KINDA_SMALL_NUMBER)
				{
					// Maintain horizontal velocity
				...
				{
					// Perform second move
					UBulletAsyncMovementUtils::TestDepenetratingMove(MovingComps, LocationInProgress, LocationInProgress + SlideDelta, TargetRotation, TargetRotation, /*bShouldSweep*/ true, InOutHit, InOutMoveRecord);
...
static FVector TestMoveToAdjustToFloorInternal(const FBulletMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, bool bPushAwayOnly, FBulletFloorCheckResult& InOutCurrentFloor, FBulletMovementRecord& InOutMoveRecord)
{
	// If we have a floor check that hasn't hit anything, don't adjust height.
	...
	if (OldFloorDist < UE::FloorQueryUtility::MIN_FLOOR_DIST || OldFloorDist > UE::FloorQueryUtility::MAX_FLOOR_DIST)
	{
		...
		UBulletAsyncMovementUtils::TestDepenetratingMove(MovingComps, Location, TargetAdjustedLocation, Rotation, Rotation, /*bShouldSweep*/ true, AdjustHit, InOutMoveRecord);
...
FVector UBulletGroundMovementUtils::TestMoveToAdjustToFloor(const FBulletMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, FBulletFloorCheckResult& InOutCurrentFloor, FBulletMovementRecord& InOutMoveRecord)
{
...
FVector UBulletGroundMovementUtils::TestMoveToKeepMinHeightAboveFloor(const FBulletMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, UPARAM(ref) FBulletFloorCheckResult& InOutCurrentFloor, UPARAM(ref) FBulletMovementRecord& InOutMoveRecord)
{
...
/* static */
bool UBulletGroundMovementUtils::TestMoveToStepOver(const FBulletMovingComponentSet& MovingComps, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, float FloorSweepDistance, const FVector& MoveDelta, const FQuat& Rotation, const FHitResult& MoveHitResult, const FBulletFloorCheckResult& CurrentFloor, bool bIsFalling, FBulletOptionalFloorCheckResult* OutFloorTestResult, FVector& OutFinalLocation, FBulletMovementRecord& InOutMoveRecord)
{
	UCapsuleComponent* CapsuleComponent = Cast<UCapsuleComponent>(MovingComps.UpdatedPrimitive.Get());

	if (!UBulletGroundMovementUtils::CanStepUpOnHitSurface(MoveHitResult) || MaxStepHeight <= 0.f)
	{
		return false;
	}
	...
	const bool bDidStepUp = UBulletAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + UpAdjustment, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, SweepUpHit);

	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TestMoveToStepOver Up: %s (role %i) UpAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *UpAdjustment.ToCompactString(), bDidStepUp);
	...
	// Cache upwards substep
	QueuedSubsteps.Add(FBulletMovementSubstep(StepUpSubstepName, UpStepDelta, false));
	LocationInProgress += UpStepDelta;

	// step fwd
	FHitResult StepFwdHit(1.f);
	const bool bDidStepFwd = UBulletAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + MoveDelta, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, StepFwdHit);

	FVector FwdStepDelta = (StepFwdHit.TraceStart + ((StepFwdHit.TraceEnd - StepFwdHit.TraceStart) * StepFwdHit.Time)) - LocationInProgress;

	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TestMoveToStepOver Fwd: %s (role %i) MoveDelta=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *MoveDelta.ToCompactString(), bDidStepFwd);
	...
	if (StepFwdHit.bBlockingHit)
	{
		...
		// Cache forwards substep before the slide attempt
		QueuedSubsteps.Add(FBulletMovementSubstep(StepFwdSubstepName, FwdStepDelta, true));
		...
		QueuedSubsteps.Add(FBulletMovementSubstep(SlideSubstepName, FwdSlideDelta, true));
	...
	const bool bDidStepDown = UBulletAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + StepDownAdjustment, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, StepFwdHit);


	UE_LOG(LogBulletMover, VeryVerbose, TEXT("TestMoveToStepOver Down: %s (role %i) StepDownAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *StepDownAdjustment.ToCompactString(), bDidStepDown);
	...
	if (StepFwdHit.IsValidBlockingHit())
	{
		...
		// Don't step up onto invalid surfaces if traveling higher.
		if (DeltaDot > 0.f && !UBulletGroundMovementUtils::CanStepUpOnHitSurface(StepFwdHit))
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being up onto surface with !CanStepUpOnHitSurface"));
			return false;
		}
		...
		if (OutFloorTestResult != NULL)
		{
			...
			UBulletFloorQueryUtils::FindFloor(MovingComps, FloorSweepDistance, MaxWalkSlopeCosine, bUseFlatBaseForFloorChecks, LocationInProgress, StepDownResult.FloorTestResult);
	...
	// Cache downwards substep
	QueuedSubsteps.Add(FBulletMovementSubstep(StepDownSubstepName, StepDownDelta, false));
