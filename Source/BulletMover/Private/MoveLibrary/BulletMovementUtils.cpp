...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletMovementUtils)
...
bool UBulletMovementUtils::IsExceedingMaxSpeed(const FVector& Velocity, float InMaxSpeed)
{
...
FVector UBulletMovementUtils::ComputeVelocity(const FComputeVelocityParams& InParams)
{
	FVector Acceleration = FVector::ZeroVector;
	FVector Velocity = InParams.InitialVelocity;
	float DesiredSpeed = 0.0f;

	if (InParams.MoveInputType == EBulletMoveInputType::Velocity)
	{
		...
		if (InParams.bUseAccelerationForVelocityMove && InParams.InitialVelocity.Size() < RequestedSpeed * UE::BulletMoverUtils::VELOCITY_INPUT_NO_ACCELERATION_DIFFERENCE)
		{
	...
	else if (InParams.MoveInputType == EBulletMoveInputType::DirectionalIntent)
	{
	...
	{
		UE_CLOG((InParams.MoveInputType == EBulletMoveInputType::Invalid), LogBulletMover, Warning, TEXT("BulletMover Compute Velocity has received an invalid input type and no velocity will be generated!"));
...
FVector UBulletMovementUtils::ComputeCombinedVelocity(const FComputeCombinedVelocityParams& InParams)
{
	const FVector ControlAcceleration = InParams.MoveDirectionIntent.GetClampedToMaxSize(1.f);
	FVector Velocity = InParams.InitialVelocity;
...
FVector UBulletMovementUtils::DeduceUpDirectionFromGravity(const FVector& GravityAcceleration)
{
	const FVector DeducedUpDir = -GravityAcceleration.GetSafeNormal();

	if (DeducedUpDir.IsZero())
	{
		return BulletMoverComponentConstants::DefaultUpDir;
	}

	return DeducedUpDir;
}

bool UBulletMovementUtils::CanEscapeGravity(const FVector& PriorVelocity, const FVector& NewVelocity, const FVector& GravityAccel, float DeltaSeconds)
{
...
FVector UBulletMovementUtils::ConstrainToPlane(const FVector& Vector, const FPlane& MovementPlane, bool bMaintainMagnitude)
{
...
FRotator UBulletMovementUtils::ApplyGravityToOrientationIntent(const FRotator& IntendedOrientation, const FQuat& WorldToGravity, bool bStayVertical)
{
	if (!bStayVertical)
	{
...
FVector UBulletMovementUtils::ComputeSlideDelta(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const float PctOfDeltaToMove, const FVector& Normal, const FHitResult& Hit)
{
	FVector ConstrainedNormal = Normal;

	if (MovingComps.MoverComponent.IsValid())
	{
		ConstrainedNormal = UBulletPlanarConstraintUtils::ConstrainNormalToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), Normal);
	}

	return FVector::VectorPlaneProject(Delta, ConstrainedNormal) * PctOfDeltaToMove;
}

FVector UBulletMovementUtils::ComputeTwoWallAdjustedDelta(const FBulletMovingComponentSet& MovingComps, const FVector& MoveDelta, const FHitResult& Hit, const FVector& OldHitNormal)
{
	...
	{
		const FVector DesiredDir = Delta;
		Delta = UBulletMovementUtils::ComputeSlideDelta(MovingComps, Delta, 1.f - Hit.Time, HitNormal, Hit);
...
float UBulletMovementUtils::TryMoveToSlideAlongSurface(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, FHitResult& Hit, bool bHandleImpact, FBulletMovementRecord& MoveRecord)
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
...
float UBulletMovementUtils::TryMoveToSlideAlongSurfaceNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	FBulletMovementRecord TempMoveRecord;
	return TryMoveToSlideAlongSurface(MovingComps, Delta, PctOfDeltaToMove, Rotation, Normal, Hit, bHandleImpact, TempMoveRecord);
}

float UBulletMovementUtils::TrySafeMoveAndSlideUpdatedComponent(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult& OutHit, ETeleportType Teleport, FBulletMovementRecord& MoveRecord, bool bSlideAlongSurface)
{
	...
	if (OutHit.IsValidBlockingHit())
	{
		if (bSlideAlongSurface)
		{
			...
			FBulletMoverOnImpactParams ImpactParams(DefaultModeNames::Flying, OutHit, Delta);
			MoverComponent->HandleImpact(ImpactParams);
			// Try to slide the remaining distance along the surface.
			TryMoveToSlideAlongSurface(FBulletMovingComponentSet(MoverComponent), Delta, 1.f - OutHit.Time, NewRotation, OutHit.Normal, OutHit, true, MoveRecord);
...
float UBulletMovementUtils::TrySafeMoveAndSlideUpdatedComponentNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult& OutHit, ETeleportType Teleport, bool bSlideAlongSurface)
{
	FBulletMovementRecord TempRecord;
...
bool UBulletMovementUtils::TrySafeMoveUpdatedComponent(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult& OutHit, ETeleportType Teleport, FBulletMovementRecord& MoveRecord)
{
	...
	// Scope for move flags
	{
		...
		if (UpdatedComponent)
		{
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("TrySafeMove: %s (role %i) Delta=%s DidMove=%i"), 
	...
	if (OutHit.bStartPenetrating && UpdatedComponent)
	{
		...
		if (TryMoveToResolvePenetration(MovingComps, MoveComponentFlags, RequestedAdjustment, OutHit, NewRotation, MoveRecord))
		{
			...
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("TrySafeMove retry: %s (role %i) Delta=%s DidMove=%i"),
				*GetNameSafe(UpdatedComponent->GetOwner()), UpdatedComponent->GetOwnerRole(), *Delta.ToCompactString(), bMoveResult);
		}
	}
	...
	{
		MoveRecord.Append( FBulletMovementSubstep(SafeMoveSubstepName, UpdatedComponent->GetComponentLocation()-PreviousCompPos, true) );
	}
...
bool UBulletMovementUtils::TrySafeMoveUpdatedComponentNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult& OutHit, ETeleportType Teleport)
{
	FBulletMovementRecord TempMovementRecord;
	return TrySafeMoveUpdatedComponent(MovingComps, Delta, NewRotation, bSweep, OutHit, Teleport, TempMovementRecord);
}

FVector UBulletMovementUtils::ComputePenetrationAdjustment(const FHitResult& Hit)
{
...
bool UBulletMovementUtils::TryMoveToResolvePenetration(const FBulletMovingComponentSet& MovingComps, EMoveComponentFlags MoveComponentFlags, const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotationQuat, FBulletMovementRecord& MoveRecord)
{
	USceneComponent* UpdatedComponent = MovingComps.UpdatedComponent.Get();
	...
	const FVector Adjustment = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), ProposedAdjustment);
	if (!Adjustment.IsZero() && UpdatedPrimitive)
	{
		...
		if (!bEncroached)
		{
			...
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToResolvePenetration unencroached: %s (role %i) Adjustment=%s DidMove=%i"),
				*GetNameSafe(UpdatedComponent->GetOwner()), UpdatedComponent->GetOwnerRole(), *Adjustment.ToCompactString(), bDidMove);

			MoveRecord.Append( FBulletMovementSubstep(PenetrationResolutionSubstepName, UpdatedComponent->GetComponentLocation()-OriginalCompPos, false) );
			return true;
		}
		else
		{
			...
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToResolvePenetration: %s (role %i) Adjustment=%s DidMove=%i"),
			...
			if (!bMoved && SweepOutHit.bStartPenetrating)
			{
				...
				if (SecondMTD != Adjustment && !CombinedMTD.IsZero())
				{
					...
					UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToResolvePenetration combined: %s (role %i) CombinedAdjustment=%s DidMove=%i"),
			...
			if (!bMoved)
			{
				...
				const FVector MoveDelta = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MoverComp->GetPlanarConstraint(), (Hit.TraceEnd - Hit.TraceStart));
				if (!MoveDelta.IsZero())
				{
					...
					UE_LOG(LogBulletMover, VeryVerbose, TEXT("TryMoveToResolvePenetration multiple: %s (role %i) AdjustAndMoveDelta=%s DidMove=%i"),
...
void UBulletMovementUtils::InitCollisionParams(const UPrimitiveComponent* UpdatedPrimitive, FCollisionQueryParams& OutParams, FCollisionResponseParams& OutResponseParam)
{
	if (UpdatedPrimitive)
	{
...
FVector UBulletMovementUtils::ComputeVelocityFromPositions(const FVector& FromPos, const FVector& ToPos, float DeltaSeconds)
{
	if (DeltaSeconds > 0.f)
	{ 
...
FRotator UBulletMovementUtils::ComputeAngularVelocity(const FRotator& FromOrientation, const FRotator& ToOrientation, const FQuat& WorldToGravity, float DeltaSeconds, float TurningRateLimit)
{
...
FVector UBulletMovementUtils::ComputeAngularVelocityDegrees(const FRotator& From, const FRotator& To, float DeltaSeconds, float TurningRateLimit)
{
	if (DeltaSeconds <= UE_SMALL_NUMBER)
	{
		return FVector::ZeroVector;
...
FVector UBulletMovementUtils::ComputeDirectionIntent(const FVector& MoveInput, EBulletMoveInputType MoveInputType, float MaxSpeed)
{
...
bool UBulletMovementUtils::IsAngularVelocityZero(const FRotator& AngularVelocity)
{
	return (AngularVelocity.Yaw == 0.0 && AngularVelocity.Pitch == 0.0 && AngularVelocity.Roll == 0.0);
}


FRotator UBulletMovementUtils::ApplyAngularVelocity(const FRotator& StartingOrient, const FRotator& AngularVelocity, float DeltaSeconds)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	if (!UBulletMovementUtils::IsAngularVelocityZero(AngularVelocity))
	{
...
FRotator UBulletMovementUtils::ApplyAngularVelocityToRotator(const FRotator& StartingOrient, const FVector& AngularVelocityDegrees, float DeltaSeconds)
{
	return ApplyAngularVelocityToQuat(StartingOrient.Quaternion(), AngularVelocityDegrees, DeltaSeconds).Rotator();
}

FQuat UBulletMovementUtils::ApplyAngularVelocityToQuat(const FQuat& StartingOrient, const FVector& AngularVelocityDegrees, float DeltaSeconds)
{
...
bool UBulletMovementUtils::FindTeleportSpot(const UBulletMoverComponent* MoverComp, FVector& TestLocation, FRotator TestRotation)
{
...
bool UBulletMovementUtils::TestEncroachment(const UBulletMoverComponent* MoverComp, FVector TestLocation, FRotator TestRotation)
{
...
bool UBulletMovementUtils::TestEncroachmentAndAdjust(const UBulletMoverComponent* MoverComp, FVector TestLocation, FRotator TestRotation, OUT FVector& OutProposedAdjustment)
{
...
bool UBulletMovementUtils::TryMoveUpdatedComponent_Internal(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, EMoveComponentFlags MoveComponentFlags, FHitResult* OutHit, ETeleportType Teleport)
{
	if (MovingComps.UpdatedComponent.IsValid())
	{
		...
		if (MovingComps.MoverComponent.IsValid())
		{
			ConstrainedDelta = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), Delta);	
		}
...
bool UBulletMovementUtils::TestEncroachment_Internal(const UWorld* World, const AActor* TestActor, const UPrimitiveComponent* PrimComp, const FTransform& TestWorldTransform, const TArray<AActor*>& IgnoreActors)
{
	...
	if (World && PrimComp)
	{
		...
		if (CollisionShape.IsBox() && (Cast<UBoxComponent>(PrimComp) == nullptr))
		{
			...
			{
				UE_LOG(LogBulletMover, Log, TEXT("Components must be registered in order to be used in a ComponentOverlapMulti call. PriComp: %s TestActor: %s"), *PrimComp->GetName(), *TestActor->GetName());
				return false;
			}
		}
		else
...
bool UBulletMovementUtils::TestEncroachmentWithAdjustment_Internal(const UWorld* World, const AActor* TestActor, const UPrimitiveComponent* PrimComp, const FTransform& TestWorldTransform, const TArray<AActor*>& IgnoreActors, OUT FVector& OutProposedAdjustment)
{
		...
		if (bFoundBlockingHit && bComputePenetrationAdjustment)
		{
			...
			for (int32 HitIdx = 0; HitIdx < Overlaps.Num(); HitIdx++)
			{
				...
				if (OverlapComponent && OverlapComponent->GetCollisionResponseToChannel(BlockingChannel) == ECR_Block)
				{
					...
					{
						UE_LOG(LogBulletMover, Log, TEXT("OverlapTest says we are overlapping, yet MTD says we're not. Something is wrong"));
					...
					if (bSuccess && FMath::IsNearlyZero(MTDResult.Distance))
					{
						...
						{
							// Ignore this overlap.
							UE_LOG(LogBulletMover, Log, TEXT("OverlapTest says we are overlapping, yet MTD says we're not (with smaller shape). Ignoring this overlap."));
							NumBlockingHits--;
							continue;
						}
					}
