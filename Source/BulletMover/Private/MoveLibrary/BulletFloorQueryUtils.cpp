...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletFloorQueryUtils)
...
void UBulletFloorQueryUtils::ComputeFloorDist(const FBulletMovingComponentSet& MovingComps, float LineTraceDistance, float FloorSweepDistance, float MaxWalkSlopeCosine, const FVector& Location, bool bUseFlatBaseForFloorChecks, FBulletFloorCheckResult& OutFloorResult)
{
	OutFloorResult.Clear();

	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(ComputeFloorDist), false, MovingComps.UpdatedPrimitive->GetOwner());
	FCollisionResponseParams ResponseParam;
	UBulletMovementUtils::InitCollisionParams(MovingComps.UpdatedPrimitive.Get(), QueryParams, ResponseParam);
...
bool UBulletFloorQueryUtils::FloorSweepTest(const FBulletMovingComponentSet& MovingComps, FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel, const struct FCollisionShape& CollisionShape, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParam, bool bUseFlatBaseForFloorChecks)
{
	...
	UBulletPhysicsWorldSubsystem* Subsystem = World->GetSubsystem<UBulletPhysicsWorldSubsystem>();
	if (!Subsystem)
	{
		return false;
	}
...
bool UBulletFloorQueryUtils::IsHitSurfaceWalkable(const FHitResult& Hit, const FVector& UpDirection, float MaxWalkSlopeCosine)
{
...
bool UBulletFloorQueryUtils::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, float CapsuleRadius, const FVector& UpDirection)
{
...
void FBulletFloorCheckResult::SetFromSweep(const FHitResult& InHit, const float InSweepFloorDist, const bool bIsWalkableFloor)
{
...
void FBulletFloorCheckResult::SetFromLineTrace(const FHitResult& InHit, const float InSweepFloorDist, const float InLineDist, const bool bIsWalkableFloor)
{
