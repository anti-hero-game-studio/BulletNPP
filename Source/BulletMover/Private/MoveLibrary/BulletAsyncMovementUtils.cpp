...
/* static */
bool UBulletAsyncMovementUtils::TestDepenetratingMove(const FBulletMovingComponentSet& MovingComps, const FVector& StartLocation, const FVector& TargetLocation, const FQuat& StartRotation, const FQuat& TargetRotation, bool bShouldSweep, FBulletMoverCollisionParams& CollisionParams, FHitResult& OutHit, FBulletMovementRecord& InOutMoveRecord)
{
	...
	// Test full move first
	{
		...
		UE_LOG(LogBulletMover, VeryVerbose, TEXT("TestDepenetratingMove: %s (role %i) Delta=%s DidMove=%i"),
	...
	if (OutHit.bStartPenetrating)
	{
		...
		if (FindMoveToResolveInitialPenetration_Internal(MovingComps, StartLocation, StartRotation, OutHit, CollisionParams, AdjustmentToResolve))
		{
			...
			InOutMoveRecord.Append(FBulletMovementSubstep(TestSafeDepenetrationSubstepName, AdjustmentToResolve, false));

			// Retry original move
			bDidMove |= TestMoveComponent_Internal(MovingComps, ResolvedStartLocation, TargetLocation, StartRotation, TargetRotation, bShouldSweep, CollisionParams, OutHit);

			UE_LOG(LogBulletMover, VeryVerbose, TEXT("TestDepenetratingMove retry: %s (role %i) Delta=%s DidMove=%i"),
				*GetNameSafe(UpdatedComponent->GetOwner()), UpdatedComponent->GetOwnerRole(), *(TargetLocation - ResolvedStartLocation).ToCompactString(), bDidMove);
	...
	if (bDidMove)
	{
		InOutMoveRecord.Append(FBulletMovementSubstep(TestSafeMoveSubstepName, CurrentLocation - ResolvedStartLocation, true));
	}
...
/* static */
float UBulletAsyncMovementUtils::TestSlidingMoveAlongHitSurface(const FBulletMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, FHitResult& InOutHit, FBulletMovementRecord& InOutMoveRecord)
{
...
/* static */ 
float UBulletAsyncMovementUtils::TestSlidingMoveAlongHitSurface(const FBulletMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, FBulletMoverCollisionParams& CollisionParams, FHitResult& InOutHit, FBulletMovementRecord& InOutMoveRecord)
{
...
/* static */
bool UBulletAsyncMovementUtils::FindMoveToResolveInitialPenetration_Internal(const FBulletMovingComponentSet& MovingComps, const FVector& StartLocation, const FQuat& StartRotation, const FHitResult& PenetratingHit, FBulletMoverCollisionParams& CollisionParams, FVector& OutAdjustmentDelta)
{
	if (ensureMsgf(PenetratingHit.bStartPenetrating, TEXT("Expected a hit that started penetrating. Will not attempt adjustment.")))
	{
		...
		FVector ProposedAdjustment = UBulletMovementUtils::ComputePenetrationAdjustment(PenetratingHit);
		ProposedAdjustment = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), ProposedAdjustment);
		...
		if (!ProposedAdjustment.IsZero())
		{
			...
			bool bEncroached = UBulletMovementUtils::OverlapTest(UpdatedComponent, UpdatedPrimitive, PenetratingHit.TraceStart + ProposedAdjustment, StartRotation, UpdatedPrimitive->GetCollisionObjectType(), UpdatedPrimitive->GetCollisionShape(OverlapInflation), ActorOwner);
			...
			UE_LOG(LogBulletMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal: %s (role %i) Adjustment=%s DidMove=%i"),
			...
			if (!bMoved && SweepOutHit.bStartPenetrating)
			{
				...
				const FVector SecondMTD = UBulletMovementUtils::ComputePenetrationAdjustment(SweepOutHit);
				...
				if (SecondMTD != ProposedAdjustment && !CombinedMTD.IsZero())
				{
					...
					UE_LOG(LogBulletMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal combined: %s (role %i) CombinedAdjustment=%s DidMove=%i"),
			...
			if (!bMoved)
			{
				const UBulletMoverComponent* MoverComp = MovingComps.MoverComponent.Get();
				...
				const FVector FullMoveDelta = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MoverComp->GetPlanarConstraint(), (PenetratingHit.TraceEnd - PenetratingHit.TraceStart));
				if (!FullMoveDelta.IsZero())
				{
					...
					UE_LOG(LogBulletMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal multiple: %s (role %i) TargetLocationWithAdjustment=%s DidMove=%i"),
...
bool UBulletAsyncMovementUtils::TestMoveComponent_Internal(const FBulletMovingComponentSet& MovingComps, const FVector& StartLocation, const FVector& TargetLocation, const FQuat& StartRotation, const FQuat& TargetRotation, bool bShouldSweep, FBulletMoverCollisionParams& CollisionParams, FHitResult& OutHit)
{
	...
	{
		ConstrainedDelta = UBulletPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), ConstrainedDelta);
	}
