...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletWaterMovementUtils)

FBulletProposedMove UBulletWaterMovementUtils::ComputeControlledWaterMove(const FBulletWaterMoveParams& InParams)
{
	...
	OutMove.LinearVelocity = UBulletMovementUtils::ComputeCombinedVelocity(ComputeCombinedVelocityParams);
	...
	OutMove.AngularVelocityDegrees = UBulletMovementUtils::ComputeAngularVelocityDegrees(InParams.PriorOrientation, IntendedFacingDir.ToOrientationRotator(), InParams.DeltaSeconds, InParams.TurningRate);

	return OutMove;
}

void UBulletWaterMovementUtils::UpdateWaterSplineData(const FBulletUpdateWaterSplineDataParams& UpdateWaterSplineDataParams, FBulletWaterCheckResult& WaterCheckResult)
{
	if(WaterCheckResult.HitResult.Component.IsValid())
	{
		if (AWaterBody* WaterBody = Cast<AWaterBody>(WaterCheckResult.HitResult.Component->GetOwner()))
		{
			if (WaterBody->GetWaterBodyComponent())
			{
				...
				FBulletWaterFlowSplineData& WaterSplineData = WaterCheckResult.WaterSplineData;
