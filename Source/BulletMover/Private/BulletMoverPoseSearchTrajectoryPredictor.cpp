...
#include "BulletMoverPoseSearchTrajectoryPredictor.h"
 #include "Animation/TrajectoryTypes.h"
 #include "BulletMoverComponent.h"
 #include "MoveLibrary/BulletMovementUtils.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletMoverPoseSearchTrajectoryPredictor)
 
void UBulletMoverTrajectoryPredictor::Predict(FTransformTrajectory& InOutTrajectory,
	int32 NumPredictionSamples, float SecondsPerPredictionSample, int NumHistorySamples)
{
	if (!MoverComponent)
	{
		UE_LOG(LogBulletMover, Log, TEXT("Calling Predict without a Mover Component. This is invalid and the trajectory will not be modified."));
		return;
	}
...
void UBulletMoverTrajectoryPredictor::Predict(UBulletMoverComponent& MoverComponent, FTransformTrajectory& InOutTrajectory, int32 NumPredictionSamples, float SecondsPerPredictionSample, int32 NumHistorySamples, float MoverSamplingInterval)
{
	FBulletMoverPredictTrajectoryParams PredictParams;
	...
	TArray<FBulletTrajectorySampleInfo> MoverPredictionSamples = MoverComponent.GetPredictedTrajectory(PredictParams);

	if (InOutTrajectory.Samples.Num() < (NumHistorySamples + NumPredictionSamples))
	{
		UE_LOG(LogBulletMover, Warning, TEXT("InOutTrajectory Samples array does not have enough space for %i predicted samples"), MoverPredictionSamples.Num());
	}
	else
	{
...
void UBulletMoverTrajectoryPredictor::GetGravity(FVector& OutGravityAccel)
{
	if (!MoverComponent)
	{
		UE_LOG(LogBulletMover, Log, TEXT("Calling GetGravity without a Mover Component. Return value will be defaulted."));
...
void UBulletMoverTrajectoryPredictor::GetCurrentState(FVector& OutPosition, FQuat& OutFacing, FVector& OutVelocity)
{
	if (!MoverComponent)
	{
		UE_LOG(LogBulletMover, Log, TEXT("Calling GetCurrentState without a Mover Component. Return values will be defaulted."));
...
void UBulletMoverTrajectoryPredictor::GetCurrentState(UBulletMoverComponent& MoverComponent, FVector& OutPosition, FQuat& OutFacing, FVector& OutVelocity)
{
...
void UBulletMoverTrajectoryPredictor::GetVelocity(FVector& OutVelocity)
{
	if (!MoverComponent)
	{
		UE_LOG(LogBulletMover, Log, TEXT("Calling GetVelocity without a Mover Component. Return value will be defaulted."));
