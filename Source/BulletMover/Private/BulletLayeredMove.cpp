...
void FBulletLayeredMoveBase::StartMove(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs)
{
	StartSimTimeMs = CurrentSimTimeMs;
	OnStart(MoverComp, SimBlackboard);
}

void FBulletLayeredMoveBase::StartMove_Async(UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs)
{
	StartSimTimeMs = CurrentSimTimeMs;
	OnStart_Async(SimBlackboard);
}

bool FBulletLayeredMoveBase::GenerateMove_Async(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, UBulletMoverBlackboard* SimBlackboard, FBulletProposedMove& OutProposedMove)
{
...
void FBulletLayeredMoveBase::EndMove(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs)
{
	OnEnd(MoverComp, SimBlackboard, CurrentSimTimeMs);
}

void FBulletLayeredMoveBase::EndMove_Async(UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs)
{
...
TArray<TSharedPtr<FBulletLayeredMoveBase>> FBulletLayeredMoveGroup::GenerateActiveMoves(const FBulletMoverTimeStep& TimeStep, const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard)
{
...
TArray<TSharedPtr<FBulletLayeredMoveBase>> FBulletLayeredMoveGroup::GenerateActiveMoves_Async(const FBulletMoverTimeStep& TimeStep, UBulletMoverBlackboard* SimBlackboard)
{
	const double SimStartTimeMs		= TimeStep.BaseSimTimeMs;
	const double SimTimeAfterTickMs	= SimStartTimeMs + TimeStep.StepMs;
...
static void CopyLayeredMoveArray(TArray<TSharedPtr<FBulletLayeredMoveBase>>& Dest, const TArray<TSharedPtr<FBulletLayeredMoveBase>>& Src)
{
	...
	if (!bCanCopyInPlace)
	{
		...
		for (int i = 0; i < Src.Num(); ++i)
		{
			...
			{
				UE_LOG(LogBulletMover, Warning, TEXT("CopyLayeredMoveArray trying to copy invalid Other Layered Move"));
			}
		}
	}
}
...
void FBulletLayeredMoveGroup::FlushMoveArrays(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs, bool bIsAsync)
{
	if (bIsAsync)
	{
		ensureMsgf(MoverComp == nullptr, TEXT("In async mode, no mover Component should be passed in as argument in FBulletLayeredMoveGroup::FlushMoveArrays"));
		MoverComp = nullptr;
	}
	...
	// Process any cancellations
	{
		for (TPair<FGameplayTag, bool> CancelRequest : TagCancellationRequests)
		{
			...
			ActiveLayeredMoves.RemoveAll([MoverComp, SimBlackboard, CurrentSimTimeMs, &bResidualVelocityOverridden, &bClampVelocityOverridden, bIsAsync, TagToMatch, bRequireExactMatch, this] (const TSharedPtr<FBulletLayeredMoveBase>& Move)
				{
...
void FBulletLayeredMoveGroup::GatherResidualVelocitySettings(const TSharedPtr<FBulletLayeredMoveBase>& Move, bool& bResidualVelocityOverridden, bool& bClampVelocityOverridden)
{
	if (Move->FinishVelocitySettings.FinishVelocityMode == EBulletLayeredMoveFinishVelocityMode::SetVelocity)
	{
		if (Move->MixMode == EBulletMoveMixMode::OverrideVelocity)
		{
			...
			{
				UE_LOG(LogBulletMover, Log, TEXT("Multiple LayeredMove residual settings have a MixMode that overrides. Only one will take effect."));
			}
...
/* static */ void FBulletLayeredMoveGroup::NetSerializeLayeredMovesArray(FArchive& Ar, TArray< TSharedPtr<FBulletLayeredMoveBase> >& LayeredMovesArray, uint8 MaxNumLayeredMovesToSerialize /*=MAX_uint8*/)
{
	...
	if (Ar.IsSaving())
	{
		UE_CLOG(LayeredMovesArray.Num() > MaxNumLayeredMovesToSerialize, LogBulletMover, Warning, TEXT("Too many Layered Moves (%d!) to net serialize. Clamping to %d"),
	...
	for (int32 i = 0; i < NumMovesToSerialize && !Ar.IsError(); ++i)
	{
		...
		if (ScriptStruct.IsValid())
		{
			...
			{
				UE_LOG(LogBulletMover, Error, TEXT("FBulletLayeredMoveGroup::NetSerialize: ScriptStruct not derived from FBulletLayeredMoveBase attempted to serialize."));
				Ar.SetError();
				break;
			}
		}
		...
		{
			UE_LOG(LogBulletMover, Error, TEXT("FBulletLayeredMoveGroup::NetSerialize: Invalid ScriptStruct serialized."));
			Ar.SetError();
			break;
		}
	}
