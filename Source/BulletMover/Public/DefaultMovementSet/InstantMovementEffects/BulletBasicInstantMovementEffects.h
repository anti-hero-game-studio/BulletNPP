...
#include "BulletBasicInstantMovementEffects.generated.h"
 ...
 */
USTRUCT(BlueprintType, Category = "Bullet Mover|Instant Movement Effect", DisplayName = "Teleport Instant Movement Effect")
struct FBulletTeleportEffect : public FBulletInstantMovementEffect
{
	GENERATED_BODY()

	UE_API FBulletTeleportEffect();
	virtual ~FBulletTeleportEffect() {}
	...
	UE_API virtual bool ApplyMovementEffect(FBulletApplyMovementEffectParams& ApplyEffectParams, FBulletMoverSyncState& OutputState) override;
	UE_API virtual bool ApplyMovementEffect_Async(FBulletApplyMovementEffectParams_Async& ApplyEffectParams, FBulletMoverSyncState& OutputState) override;

	UE_API virtual FBulletInstantMovementEffect* Clone() const override;
...
USTRUCT(BlueprintType, Category = "Bullet Mover|Instant Movement Effect", DisplayName = "Async Teleport Instant Movement Effect")
struct FAsyncTeleportEffect : public FBulletTeleportEffect
{
	GENERATED_BODY()
...
/** Jump Impulse: introduces an instantaneous upwards change in velocity. This overrides the existing 'up' component of the actor's current velocity
  ...
  */
USTRUCT(BlueprintType, Category = "Bullet Mover|Instant Movement Effect", DisplayName = "Jump Impulse Instant Movement Effect")
struct FJumpImpulseEffect : public FBulletInstantMovementEffect
{
	GENERATED_BODY()
...
/** Apply Velocity: provides an impulse velocity to the actor after (optionally) forcing them into a particular movement mode
  * Note: this only applies the impulse for one tick!
  */
USTRUCT(BlueprintType, Category = "Bullet Mover|Instant Movement Effect", DisplayName = "Apply Velocity Instant Movement Effect")
struct FBulletApplyVelocityEffect : public FBulletInstantMovementEffect
{
	GENERATED_BODY()

	UE_API FBulletApplyVelocityEffect();
	virtual ~FBulletApplyVelocityEffect() {}
