...
#include "BulletRollbackBlackboard.generated.h"
 ...
 */
UENUM()
enum class EBulletBlackboardSizingPolicy : uint8
{
 ...
 */
UENUM()
enum class EBulletBlackboardPersistencePolicy : uint8
{
 ...
 */
UENUM()
enum class EBulletBlackboardRollbackPolicy : uint8
{
 ...
 * See @UBulletRollbackBlackboard_InternalWrapper for in-simulation use.
 */ 
UCLASS(MinimalAPI)
class UBulletRollbackBlackboard : public UObject
{
	GENERATED_BODY()
...
public:
	...
	{
		EBulletBlackboardSizingPolicy      SizingPolicy      = EBulletBlackboardSizingPolicy::FixedDeclaredSize;
		EBulletBlackboardPersistencePolicy PersistencePolicy = EBulletBlackboardPersistencePolicy::Forever;
		EBulletBlackboardRollbackPolicy    RollbackPolicy    = EBulletBlackboardRollbackPolicy::InvalidatedOnRollback;
...
private:
	...
	public:
		...
		UE_API static uint32 ComputeBufferSize(EBulletBlackboardSizingPolicy InSizingPolicy, uint32 FixedBufferSize = -1);
...
public:
	...
	bool CreateEntry(FName EntryName, const EntrySettings& InSettings)
	{
		...
		{
			UE_LOG(LogBulletMover, Warning, TEXT("Skipping attempt to create a new blackboard entry named %s since it already exists. Policy settings from the first entry will be retained."), *EntryName.ToString());
			return false;
		}
...
private:
	UE_API void BeginSimulationFrame(const FBulletMoverTimeStep& PendingTimeStep);
	UE_API void EndSimulationFrame();

	UE_API void BeginRollback(const FBulletMoverTimeStep& NewBaseTimeStep);	// NewBaseTimeStep represents the current time and the PENDING frame that's next to be resimulated
...
private:
	...
	friend class UBulletRollbackBlackboard_InternalWrapper;	// expose non-public API to this wrapper class
};
 ...
 */
UCLASS(MinimalAPI)
class UBulletRollbackBlackboard_InternalWrapper : public UObject
{
	GENERATED_BODY()

public:
	void Init(UBulletRollbackBlackboard& InBlackboard)
	{
		Blackboard = &InBlackboard;
	}

	template<typename EntryT>
	bool CreateEntry(FName EntryName, const UBulletRollbackBlackboard::EntrySettings& InSettings)
	{
	...
	UE_API void BeginRollback(const FBulletMoverTimeStep& NewBaseTimeStep);
	UE_API void EndRollback();

private: 
	UPROPERTY()
	TObjectPtr<UBulletRollbackBlackboard> Blackboard;
};
