...
#include "BulletConstrainedMoveUtils.generated.h"
...
struct FBulletPlanarConstraint
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UBulletPlanarConstraintUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	/** Sets whether or not the constraint is enabled */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API void SetPlanarConstraintEnabled(UPARAM(ref) FBulletPlanarConstraint& Constraint, bool bEnabled);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API void SetPlanarConstraintNormal(UPARAM(ref) FBulletPlanarConstraint& Constraint, FVector PlaneNormal);

	/** Sets the origin of the plane that constrains movement, enforced if the plane constraint is enabled. */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API void SetPlaneConstraintOrigin(UPARAM(ref) FBulletPlanarConstraint& Constraint, FVector PlaneOrigin);

	// APPLICATION OF CONSTRAINT

	/** Constrains a direction to be on the plane, if enabled */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API FVector ConstrainDirectionToPlane(const FBulletPlanarConstraint& Constraint, FVector Direction, bool bMaintainMagnitude=false);

	/** Constrains a location to be on the plane, if enabled */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API FVector ConstrainLocationToPlane(const FBulletPlanarConstraint& Constraint, FVector Location);

	/** Constrains a normal to be on the plane, if enabled. Result will be re-normalized. */
	UFUNCTION(BlueprintCallable, Category = "Bullet Mover|PlanarConstraint")
	static UE_API FVector ConstrainNormalToPlane(const FBulletPlanarConstraint& Constraint, FVector Normal);

}; // UBulletPlanarConstraintUtils
