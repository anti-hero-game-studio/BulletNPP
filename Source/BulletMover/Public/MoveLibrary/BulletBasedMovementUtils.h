...
#include "BulletBasedMovementUtils.generated.h"
...
struct FBulletRelativeBaseInfo
{
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadWrite, Category="BulletMover")
...
public:
	...
	UE_API bool UsesSameBase(const FBulletRelativeBaseInfo& Other) const;
	UE_API bool UsesSameBase(const UPrimitiveComponent* OtherComp, FName OtherBoneName=NAME_None) const;

	UE_API void SetFromFloorResult(const FBulletFloorCheckResult& FloorTestResult);
 ...
 */
UCLASS(MinimalAPI)
class UBulletBasedMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	/** Determine whether MovementBase can possibly move. */
	UFUNCTION(BlueprintCallable, Category = "BulletMover/MovementBases")
	...
	static UE_API void UpdateSimpleBasedMovement(UBulletMoverComponent* TargetMoverComp);
};
 ...
 **/
USTRUCT()
struct FBulletMoverDynamicBasedMovementTickFunction : public FTickFunction
{
	GENERATED_BODY()

	/** MoverComponent that is the target of this tick **/
	UBulletMoverComponent* TargetMoverComp;
...
template<>
struct TStructOpsTypeTraits<FBulletMoverDynamicBasedMovementTickFunction> : public TStructOpsTypeTraitsBase2<FBulletMoverDynamicBasedMovementTickFunction>
{
	enum
	{
		WithCopy = false
