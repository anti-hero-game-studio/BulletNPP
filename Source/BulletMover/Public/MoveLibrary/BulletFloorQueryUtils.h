...
#include "BulletFloorQueryUtils.generated.h"
...
struct FBulletOptionalFloorCheckResult
{
	...
	FBulletFloorCheckResult FloorTestResult;

	FBulletOptionalFloorCheckResult()
	{
 ...
 */
UCLASS(MinimalAPI)
class UBulletFloorQueryUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	static UE_API void FindFloor(const FBulletMovingComponentSet& MovingComps, float FloorSweepDistance, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, const FVector& Location, FBulletFloorCheckResult& OutFloorResult);

	static UE_API void ComputeFloorDist(const FBulletMovingComponentSet& MovingComps, float LineTraceDistance, float FloorSweepDistance, float MaxWalkSlopeCosine, const FVector& Location, bool bUseFlatBaseForFloorChecks, FBulletFloorCheckResult& OutFloorResult);

	static UE_API bool FloorSweepTest(const FBulletMovingComponentSet& MovingComps, FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel, const FCollisionShape& CollisionShape, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParam, bool bUseFlatBaseForFloorChecks);
