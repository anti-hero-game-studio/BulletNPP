...
#include "BulletMovementUtils.generated.h"
...
{
	BULLETMOVER_API extern const double SMALL_MOVE_DISTANCE;
	BULLETMOVER_API extern const double VERTICAL_SLOPE_NORMAL_MAX_DOT;
}

/** Encapsulates detailed trajectory sample info, from a move that has already occurred or one projected into the future */
USTRUCT(BlueprintType)
struct FBulletTrajectorySampleInfo
{
	GENERATED_BODY()
...
struct FComputeVelocityParams
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Mover)
	EBulletMoveInputType MoveInputType = EBulletMoveInputType::DirectionalIntent;
 ...
 */
UCLASS(MinimalAPI)
class UBulletMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category=Mover)
	static UE_API FVector ComputeSlideDelta(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const float PctOfDeltaToMove, const FVector& Normal, const FHitResult& Hit);

	/** Returns an alternative move delta when we are in contact with 2 surfaces */
	static UE_API FVector ComputeTwoWallAdjustedDelta(const FBulletMovingComponentSet& MovingComps, const FVector& MoveDelta, const FHitResult& Hit, const FVector& OldHitNormal);

	/**
	 * Attempts to move a component along a surface. Returns the percent of time applied, with 0.0 meaning no movement occurred.
	 * Note: This function takes a movement record that collects moves applied to the actor see @FBulletMovementRecord
	 */
	UFUNCTION(BlueprintCallable, Category=Mover, meta=(DisplayName="Try Move To Slide Along Surface (Movement Record)"))
	static UE_API float TryMoveToSlideAlongSurface(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, UPARAM(Ref) FHitResult& Hit, bool bHandleImpact, UPARAM(Ref) FBulletMovementRecord& MoveRecord);
	...
	static UE_API float TryMoveToSlideAlongSurfaceNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, UPARAM(Ref) FHitResult& Hit, bool bHandleImpact);
	...
	static UE_API float TrySafeMoveAndSlideUpdatedComponent(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, UPARAM(Ref) FBulletMovementRecord& MoveRecord, bool bSlideAlongSurface = true);
	...
	static UE_API float TrySafeMoveAndSlideUpdatedComponentNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, bool bSlideAlongSurface = true);
	...
	static UE_API bool TrySafeMoveUpdatedComponent(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, UPARAM(Ref) FBulletMovementRecord& MoveRecord);
	...
	static UE_API bool TrySafeMoveUpdatedComponentNoMovementRecord(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport);
	...
	static UE_API bool TryMoveToResolvePenetration(const FBulletMovingComponentSet& MovingComps, EMoveComponentFlags MoveComponentFlags, const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotationQuat, FBulletMovementRecord& MoveRecord);
	...
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API FVector ComputeDirectionIntent(const FVector& MoveInput, EBulletMoveInputType MoveInputType, float MaxSpeed);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API bool FindTeleportSpot(const UBulletMoverComponent* MoverComp, UPARAM(Ref) FVector& TestLocation, FRotator TestRotation);

	/** Returns whether MoverComp's Actor would encroach at TestLocation on something that blocks it. */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API bool TestEncroachment(const UBulletMoverComponent* MoverComp, FVector TestLocation, FRotator TestRotation);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API bool TestEncroachmentAndAdjust(const UBulletMoverComponent* MoverComp, FVector TestLocation, FRotator TestRotation, FVector& OutProposedAdjustment);
	 ...
	 */
	static UE_API bool TryMoveUpdatedComponent_Internal(const FBulletMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, EMoveComponentFlags MoveComponentFlags, FHitResult* OutHit, ETeleportType Teleport);
...
template <class ComponentType>
const ComponentType* UBulletMovementUtils::GetOriginalComponentType(const AActor* MoverCompOwner)
{
