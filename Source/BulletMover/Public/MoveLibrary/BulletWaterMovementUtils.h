...
#include "BulletWaterMovementUtils.generated.h"
...
USTRUCT(BlueprintType)
struct FBulletWaterFlowSplineData
{
	GENERATED_BODY()
	
	FBulletWaterFlowSplineData()
	{
		Reset();
	}
...
USTRUCT(BlueprintType)
struct FBulletWaterCheckResult
{
	GENERATED_BODY()
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Category = CharacterVolume)
	FBulletWaterFlowSplineData WaterSplineData; 
	
public:
	FBulletWaterCheckResult()
...
USTRUCT(BlueprintType)
struct FBulletUpdateWaterSplineDataParams
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UBulletWaterMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static UE_API FBulletProposedMove ComputeControlledWaterMove(const FBulletWaterMoveParams& InParams);
	
	/** Updates the water spline data used in calculated swimming movement */
	UFUNCTION(BlueprintCallable, Category=Mover)
	static UE_API void UpdateWaterSplineData(const FBulletUpdateWaterSplineDataParams& UpdateWaterSplineDataParams, UPARAM(ref) FBulletWaterCheckResult& WaterCheckResult);
};
