...
#include "BulletAirMovementUtils.generated.h"
 ...
 */
UCLASS(MinimalAPI)
class UBulletAirMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static UE_API FBulletProposedMove ComputeControlledFreeMove(const FBulletFreeMoveParams& InParams);
	
    // Checks if a hit result represents a walkable location that an actor can land on
    UFUNCTION(BlueprintCallable, Category=Mover)
    static UE_API bool IsValidLandingSpot(const FBulletMovingComponentSet& MovingComps, const FVector& Location, const FHitResult& Hit, float FloorSweepDistance, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, UPARAM(Ref) FBulletFloorCheckResult& OutFloorResult);
...
public:	
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API float TestFallingMoveAlongHitSurface(const FBulletMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, bool bHandleImpact, float FloorSweepDistance, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, UPARAM(Ref) FHitResult& InOutHit, UPARAM(Ref) FBulletFloorCheckResult& OutFloorResult, UPARAM(Ref) FBulletMovementRecord& InOutMoveRecord);
