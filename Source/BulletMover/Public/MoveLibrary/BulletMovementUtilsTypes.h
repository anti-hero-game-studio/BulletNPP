...
#include "BulletMovementUtilsTypes.generated.h"
...
struct FBulletMovingComponentSet
{
	...
	FBulletMovingComponentSet(UBulletMoverComponent* InMoverComponent) { SetFrom(InMoverComponent); }

	UE_API void SetFrom(USceneComponent* InUpdatedComponent);
	UE_API void SetFrom(UBulletMoverComponent* InMoverComponent);
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Mover)
	TWeakObjectPtr<UBulletMoverComponent> MoverComponent;
};
 ...
 */
struct FBulletMoverCollisionParams
{
public: 
	FBulletMoverCollisionParams() {}
	UE_API FBulletMoverCollisionParams(const USceneComponent* SceneComp);
