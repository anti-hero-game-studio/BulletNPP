...
struct FBulletLayeredMoveBase
{
	...
	UE_API void StartMove(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	// Async version of the above
	UE_API void StartMove_Async(UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);

	// Called when this layered move starts.
	virtual void OnStart(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard) {}
	// Async version of the above
	virtual void OnStart_Async(UBulletMoverBlackboard* SimBlackboard) {}

	// TODO: consider whether MoverComp should just be part of FBulletMoverTickStartData
	// Generate a movement that will be combined with other sources
	virtual bool GenerateMove(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, FBulletProposedMove& OutProposedMove) { return false; }
	// Async version of the above
	UE_API virtual bool GenerateMove_Async(const FBulletMoverTickStartData& StartState, const FBulletMoverTimeStep& TimeStep, UBulletMoverBlackboard* SimBlackboard, FBulletProposedMove& OutProposedMove);
	...
	UE_API void EndMove(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	// Async version of the above
	UE_API void EndMove_Async(UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	
	// Called when this layered move ends.
	virtual void OnEnd(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs) {}
	// Async version of the above
	virtual void OnEnd_Async(UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs) {}
...
struct FBulletLayeredMoveGroup
{
	...
	UE_API TArray<TSharedPtr<FBulletLayeredMoveBase>> GenerateActiveMoves(const FBulletMoverTimeStep& TimeStep, const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard);
	UE_API TArray<TSharedPtr<FBulletLayeredMoveBase>> GenerateActiveMoves_Async(const FBulletMoverTimeStep& TimeStep, UBulletMoverBlackboard* SimBlackboard);
	...
	UE_API const FBulletLayeredMoveBase* FindActiveMove(const UScriptStruct* LayeredMoveStructType) const;
	...
	UE_API const FBulletLayeredMoveBase* FindQueuedMove(const UScriptStruct* LayeredMoveStructType) const;
...
protected:
	...
	UE_API void FlushMoveArrays(const UBulletMoverComponent* MoverComp, UBulletMoverBlackboard* SimBlackboard, double CurrentSimTimeMs, bool bIsAsync);
	...
	static UE_API void NetSerializeLayeredMovesArray(FArchive& Ar, TArray< TSharedPtr<FBulletLayeredMoveBase> >& LayeredMovesArray, uint8 MaxNumLayeredMovesToSerialize = MAX_uint8);
