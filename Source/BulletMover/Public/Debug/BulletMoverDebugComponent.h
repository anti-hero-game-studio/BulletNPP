...
#include "BulletMoverDebugComponent.generated.h"
...
class UBulletMoverDebugComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	// Begin UActorComponent interface
	UE_API UBulletMoverDebugComponent();
	...
	UE_API TArray<FBulletTrajectorySampleInfo> GetPastTrajectory() const;
...
protected:
	...
	UE_API void UpdateHistoryTrackingForFrame(const FBulletMoverTimeStep& TimeStep, const FBulletMoverSyncState* SyncState, const FBulletMoverAuxStateContext* AuxState);

	UFUNCTION()
	UE_API void OnHistoryTrackingRollback(const FBulletMoverTimeStep& NewTimeStep, const FBulletMoverTimeStep& InvalidatedTimeStep);

	bool bIsTrackingHistory = false;	// Master flag for whether trajectory history is being tracked
	TRingBuffer<FBulletTrajectorySampleInfo> HistorySamples;	// Running buffer of most recent trajectory
	// End Motion history tracking
	 
private:
	// Begin Trail simulation
	UFUNCTION()
	void OnMovementSimTick(const FBulletMoverTimeStep& TimeStep);

	UFUNCTION()
	void OnMovementSimRollback(const FBulletMoverTimeStep& NewTimeStep, const FBulletMoverTimeStep& InvalidatedTimeStep);
