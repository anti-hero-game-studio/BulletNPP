...
#include "BulletMoverDataModelTypes.generated.h"

#define UE_API BULLETMOVER_API

namespace UE::BulletNetQuant
{
...
struct FBulletCharacterDefaultInputs : public FBulletMoverDataStructBase
{
	GENERATED_BODY()
...
public:
	...
	virtual ~FBulletCharacterDefaultInputs() {}

	bool operator==(const FBulletCharacterDefaultInputs& Other) const
	{
	...
	bool operator!=(const FBulletCharacterDefaultInputs& Other) const { return !operator==(Other); }
	
	// @return newly allocated copy of this FBulletCharacterDefaultInputs. Must be overridden by child classes
	...
	UE_API virtual void Merge(const FBulletMoverDataStructBase& From) override;
	UE_API virtual void Decay(float DecayAmount) override;
};

template<>
struct TStructOpsTypeTraits< FBulletCharacterDefaultInputs > : public TStructOpsTypeTraitsBase2< FBulletCharacterDefaultInputs >
{
	enum
	{
 ...
 */
UCLASS(MinimalAPI)
class UBulletMoverDataModelBlueprintLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:	// FBulletCharacterDefaultInputs

	/** Sets move input from a unit length vector representing directional intent */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API void SetDirectionalInput(UPARAM(Ref) FBulletCharacterDefaultInputs& Inputs, const FVector& DirectionInput);
	
	/** Sets move input from a vector representing desired velocity c/m^2 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API void SetVelocityInput(UPARAM(Ref) FBulletCharacterDefaultInputs& Inputs, const FVector& VelocityInput = FVector::ZeroVector);
