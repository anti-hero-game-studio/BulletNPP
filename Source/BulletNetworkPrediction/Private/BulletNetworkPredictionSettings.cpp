...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BulletNetworkPredictionSettings)

#if WITH_EDITOR
void UBulletNetworkPredictionSettingsObject::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (TObjectIterator<UBulletNetworkPredictionWorldManager> It; It; ++It)
	{
...
namespace UE_NETWORK_PREDICTION
{
	struct FDevMenuState
	{
		...
		TArray<FBulletNetworkPredictionDevHUD> DevHUDs;
	};
	...
	FORCENOINLINE void DrawDevHUD(TMap<TWeakObjectPtr<UWorld>, FDevMenuState>& WorldHandlesMap, AHUD* HUD, UCanvas* Canvas)
	{
		...
		if (State.ActiveHUD == INDEX_NONE)
		{
			...
			{
				const FBulletNetworkPredictionDevHUD& DevHUD = State.DevHUDs[idx];
		...
		{
			const FBulletNetworkPredictionDevHUD& DevHUD = State.DevHUDs[State.ActiveHUD];
			for (int32 idx=0; idx < DevHUD.Items.Num(); ++idx)
			{
				const FBulletNetworkPredictionDevHUDItem& HUDItem = DevHUD.Items[idx];
	...
	FORCENOINLINE void DevMenu(const TArray< FString >& Args, UWorld* InWorld)
	{
		...
		{
			const UBulletNetworkPredictionSettingsObject* Settings = GetDefault<UBulletNetworkPredictionSettingsObject>();
			check(Settings);
			State.DevHUDs = Settings->DevHUDs;

			ALevelScriptActor* LevelScript = InWorld->GetLevelScriptActor();
			FBulletNetworkPredictionDevHUD& LevelScriptDevHUD = State.DevHUDs.InsertDefaulted_GetRef(0);
			...
			if (LevelScript)
			{
				for (TFieldIterator<UFunction> FunctionIt(LevelScript->GetClass(), EFieldIteratorFlags::ExcludeSuper); FunctionIt; ++FunctionIt)
				{
					...
					FBulletNetworkPredictionDevHUDItem& Item = LevelScriptDevHUD.Items.AddDefaulted_GetRef();
		...
		else
		{
			if (State.DevHUDs.IsValidIndex(State.ActiveHUD))
			{
				...
				{
					const FBulletNetworkPredictionDevHUDItem& Item = DevHUD.Items[Choice-1];
