...
#include "Services/BulletNetworkPredictionService_Finalize.inl"
#include "Services/BulletNetworkPredictionService_Input.inl"
#include "Services/BulletNetworkPredictionService_Interpolate.inl"
#include "Services/BulletNetworkPredictionService_Rollback.inl"
...
UBulletNetworkPredictionWorldManager* UBulletNetworkPredictionWorldManager::ActiveInstance=nullptr;

// -----------------------------------------------------------------------------------------------
//
// -----------------------------------------------------------------------------------------------

UBulletNetworkPredictionWorldManager::UBulletNetworkPredictionWorldManager()
{

}

void UBulletNetworkPredictionWorldManager::Initialize(FSubsystemCollectionBase& Collection)
{
	UWorld* World = GetWorld();
	check(World);
	if (World->WorldType == EWorldType::PIE || World->WorldType == EWorldType::Game)
	{
		PreTickDispatchHandle = FWorldDelegates::OnWorldTickStart.AddUObject(this, &UBulletNetworkPredictionWorldManager::OnWorldPreTick);
		PostTickDispatchHandle = World->OnPostTickDispatch().AddUObject(this, &UBulletNetworkPredictionWorldManager::ReconcileSimulationsPostNetworkUpdate);
		PreWorldActorTickHandle = FWorldDelegates::OnWorldPreActorTick.AddUObject(this, &UBulletNetworkPredictionWorldManager::BeginNewSimulationFrame);
		SyncNetworkPredictionSettings(GetDefault<UBulletNetworkPredictionSettingsObject>());
	}
}

void UBulletNetworkPredictionWorldManager::Deinitialize()
{
...
void UBulletNetworkPredictionWorldManager::SyncNetworkPredictionSettings(const UBulletNetworkPredictionSettingsObject* SettingsObj)
{
	this->Settings = SettingsObj->Settings;
}

float UBulletNetworkPredictionWorldManager::GetCurrentLagCompensationTimeMS(const AActor* Actor) const
{
	...
	{
		if (Settings.PreferredTickingPolicy == EBulletNetworkPredictionTickingPolicy::Fixed)
		{
...
FNpLagCompensationData UBulletNetworkPredictionWorldManager::GetActorDefaultStateAtTime(AActor* RequestingActor,
...
void UBulletNetworkPredictionWorldManager::OnInputReceived(const int32& Frame,const float& InterpolationTime,const TArray<FBulletSimulationReplicatedInput>& Inputs
...
void UBulletNetworkPredictionWorldManager::OnReceivedAckedData(const FBulletSerializedAckedFrames& AckedFrames,
	UBulletNetworkPredictionPlayerControllerComponent* RPCHandler)
{
	...
	FBulletAckedFrames& ClientAckedFrames = FixedTickState.ServerAckedFrames.ConnectionsAckedFrames.FindOrAdd(RPCHandler->GetNetConnection());
...
void UBulletNetworkPredictionWorldManager::RegisterRPCHandler(UBulletNetworkPredictionPlayerControllerComponent* RPCHandler)
{
	RPCHandlers.AddUnique(RPCHandler);
}

void UBulletNetworkPredictionWorldManager::UnRegisterRPCHandler(UBulletNetworkPredictionPlayerControllerComponent* RPCHandler)
{
	RPCHandlers.Remove(RPCHandler);
}

void UBulletNetworkPredictionWorldManager::SetTimeDilation(const FBulletSimTimeDilation& TimeDilation)
{
...
void UBulletNetworkPredictionWorldManager::OnWorldPreTick(UWorld* InWorld, ELevelTick InLevelTick, float InDeltaSeconds)
{
	if (InWorld != GetWorld())
	{
		return;
	...
	{
		UClass* ReplicatedManagerClass = GetDefault<UBulletNetworkPredictionSettingsObject>()->Settings.ReplicatedManagerClassOverride.Get();
		ReplicatedManager = ReplicatedManagerClass ? InWorld->SpawnActor<ABulletNetworkPredictionReplicatedManager>(ReplicatedManagerClass) : InWorld->SpawnActor<ABulletNetworkPredictionReplicatedManager>();
	}
}

void UBulletNetworkPredictionWorldManager::OnWorldPreTick_Internal(float InDeltaSeconds, float InFixedFrameRate)
{
...
void UBulletNetworkPredictionWorldManager::ReconcileSimulationsPostNetworkUpdate()
{
	UWorld* World = GetWorld();
...
void UBulletNetworkPredictionWorldManager::ReconcileSimulationsPostNetworkUpdate_Internal()
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::ReconcileFixedInterpolate);
		for (TUniquePtr<IBulletFixedInterpolateService>& Ptr : Services.FixedInterpolate.Array)
		{
			Ptr->Reconcile(&FixedTickState);
		}
	}

	for (TUniquePtr<IBulletIndependentInterpolateService>& Ptr : Services.IndependentInterpolate.Array)
	{
		Ptr->Reconcile(&VariableTickState);
	}
	
	// Does anyone need to rollback?
	TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::ReconcileQueeryRollback);
	...
	// -------------------------------------------------------------------------
	for (TUniquePtr<IBulletIndependentRollbackService>& Ptr : Services.IndependentRollback.Array)
	{
		Ptr->Reconcile(&VariableTickState);
	}
...
void UBulletNetworkPredictionWorldManager::BeginNewSimulationFrame(UWorld* InWorld, ELevelTick InLevelTick, float DeltaTimeSeconds)
{
...
void UBulletNetworkPredictionWorldManager::BeginNewSimulationFrame_Internal(float DeltaTimeSeconds)
{
	...
	if (Services.FixedTick.Array.Num() > 0)
	{
		...
		while ((FixedTickState.UnspentTimeMS + KINDA_SMALL_NUMBER) >= FixedTickState.TimeDilationState.FixedStepDilatedTimeMS)
		{
			...
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::FixedSmoothing);
				for (TUniquePtr<IBulletFixedSmoothingService>& Ptr : Services.FixedSmoothing.Array)
				{
					Ptr->UpdateSmoothing(ServiceStep, &FixedTickState);
				}
			}

			for (UBulletNetworkPredictionLagCompensation* RegisteredComp : RegisteredLagCompComponents)
			{
			...
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::CallServerRPC);
				...
				if (Services.FixedServerRPC.Array.Num() > 0)
				{
					...
					{
						for (TUniquePtr<IBulletFixedServerRPCService>& Ptr : Services.FixedServerRPC.Array)
						{
						...
						for (UBulletNetworkPredictionPlayerControllerComponent*& InputHandler : RPCHandlers)
						{
							...
							{
								InputHandler->SendAckedFrames(FBulletSerializedAckedFrames(FixedTickState.LocalAckedFrames));
							}
						}
	...
	// -------------------------------------------------------------------------
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::IndependentTick);
		...
		FBulletVariableTickState::FFrame& PendingFrameData = VariableTickState.Frames[VariableTickState.PendingFrame];
	...
	// -------------------------------------------------------------------------
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::Interpolation);
		...
		if (Services.FixedInterpolate.Array.Num() > 0)
		{
			...
			if (LatestRecvFrame != INDEX_NONE)
			{
				...
				if (InterpolateRate > 0.f)
				{
					...
					{
						TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::FinalizeFrame);
						for (TUniquePtr<IBulletFixedInterpolateService>& Ptr : Services.FixedInterpolate.Array)
						{
		...
		if (Services.IndependentInterpolate.Array.Num() > 0)
		{
			...
			if (VariableTickState.Interpolation.LatestRecvTimeMS > DesiredBufferedMS)
			{
				...
				{
					UE_LOG(LogBulletNetworkPrediction, Warning, TEXT("Independent Interpolation fell behind. BufferedMS: %d"), BufferedMS);
				...
				{
					UE_LOG(LogBulletNetworkPrediction, Warning, TEXT("Independent Interpolation starved: %d"), BufferedMS);
	...
	// -------------------------------------------------------------------------
	{
		...
		for (TUniquePtr<IBulletFinalizeService>& Ptr : Services.FixedFinalize.Array)
		{
		...
		for (TUniquePtr<IBulletFinalizeService>& Ptr : Services.IndependentLocalFinalize.Array)
		{
			Ptr->FinalizeFrame(DeltaTimeSeconds, IndependentFrame, IndependentTotalSimTimeMS, 0);
		}
	
		for (TUniquePtr<IBulletRemoteFinalizeService>& Ptr : Services.IndependentRemoteFinalize.Array)
		{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(BulletNetworkPrediction::CallServerRPC);
		for (TUniquePtr<IBulletServerRPCService>& Ptr : Services.ServerRPC.Array)
		{
...
EBulletNetworkPredictionTickingPolicy UBulletNetworkPredictionWorldManager::PreferredDefaultTickingPolicy() const
{
...
void UBulletNetworkPredictionWorldManager::TickLocalPlayerControllers(ELevelTick InLevelTick, float InDeltaSeconds) const
{
...
void UBulletNetworkPredictionWorldManager::EnableLocalPlayerControllersTicking() const
{
...
void UBulletNetworkPredictionWorldManager::RegisterRewindableComponent(UBulletNetworkPredictionLagCompensation* RewindComp)
{
	if (RewindComp)
	{
...
void UBulletNetworkPredictionWorldManager::UnregisterRewindableComponent(UBulletNetworkPredictionLagCompensation* RewindComp)
{
	if (LagCompRegistrationLock > 0)
	{
...
bool UBulletNetworkPredictionWorldManager::RewindActors(AActor* RequestingActor, const float RewindSimTimeMS)
{
	if (!RequestingActor)
	{
		return false;
	...
	if (RequestingActor->GetLocalRole() == ROLE_Authority)
	{
		...
		{
			UE_LOG(LogBulletNetworkPrediction,Warning,TEXT("Desired Lag Compensation Rewind Exceeded Supported ping for %s : Desired Time %f , FinalTime %f")
		,*GetNameSafe(RequestingActor),RewindSimTimeMS,FinalRewindTime);
		}
	}
	FLagCompensationRegistrationLock(this);
	bool DidRewind = false;
	for (UBulletNetworkPredictionLagCompensation* RewindComp : RegisteredLagCompComponents)
	{
...
bool UBulletNetworkPredictionWorldManager::UnwindActors()
{
	if (RegisteredLagCompComponents.Num() <= 0)
	{
		return false;
	...
	for ( UBulletNetworkPredictionLagCompensation* RewindComp : RegisteredLagCompComponents)
	{
		...
		if (History.Num() <= 0)
		{
			UE_LOG(LogBulletNetworkPrediction, Log, TEXT("Trying To Unwind Actor %s That Has No History"),*GetNameSafe(RewindComp->GetOwner()));
			continue;
		}
...
TSharedPtr<FNpLagCompensationData> UBulletNetworkPredictionWorldManager::GetActorStateAtTime(AActor* RequestingActor,
	AActor* TargetActor, const float TargetSimTimeMS)
{
	if (!TargetActor)
	{
	...
	UBulletNetworkPredictionLagCompensation* TargetComp = TargetActor->GetComponentByClass<UBulletNetworkPredictionLagCompensation>();
	if (!TargetComp)
	{
		return nullptr;
	}
...
TSharedPtr<FNpLagCompensationData> UBulletNetworkPredictionWorldManager::GetComponentStateAtTime(AActor* RequestingActor,
								UBulletNetworkPredictionLagCompensation* TargetComp, const float TargetSimTimeMS)
{
	...
	checkf(RegisteredLagCompComponents.Num() > 0,TEXT("Trying to get actor state from non-existent component ??"
												   " this valid UBulletNetworkPredictionLagCompensation %s , didn't register with the subsystem, "
...
float UBulletNetworkPredictionWorldManager::GetInterpolationDelayMS(const UBulletNetworkPredictionWorldManager* NetworkPredictionWorldManager)
{
	if (NetworkPredictionWorldManager)
	{
		if (NetworkPredictionWorldManager->GetSettings().PreferredTickingPolicy == EBulletNetworkPredictionTickingPolicy::Fixed)
		{
...
float UBulletNetworkPredictionWorldManager::ClampRewindingTime( const float CurrentTime,const float InTargetRewindTime) const
{
	float ClampedTime = InTargetRewindTime;
	const UBulletNetworkPredictionWorldManager* NetworkPredictionWorldManager = GetWorld()->GetSubsystem<UBulletNetworkPredictionWorldManager>();
	if (!NetworkPredictionWorldManager)
	{
...
float UBulletNetworkPredictionWorldManager::GetMaxRewindDuration(const UBulletNetworkPredictionWorldManager* NetworkPredictionWorldManager) const
{
...
TSharedPtr<FNpLagCompensationData> UBulletNetworkPredictionWorldManager::GetRewindDataFromComponent(const float TargetTimeMS, const UBulletNetworkPredictionLagCompensation* LagCompComponent)
{
	if (!LagCompComponent)
	{
		return nullptr;
...
TSharedPtr<FNpLagCompensationData> UBulletNetworkPredictionWorldManager::GetLatestDataFromComponent(
	const UBulletNetworkPredictionLagCompensation* LagCompComponent)
{
	return LagCompComponent->GetLagCompensationHistory().LastCopy();
}

const TArray<UBulletNetworkPredictionLagCompensation*>& UBulletNetworkPredictionWorldManager::GetRegisteredComponents()
{
