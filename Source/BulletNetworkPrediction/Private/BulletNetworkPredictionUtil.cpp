...
namespace UE_NP
{
	Chaos::FSingleParticlePhysicsProxy* FindBestPhysicsProxy(AActor* Owner, FName NamedComponent)
	{
		...
		if (NamedComponent != NAME_None)
		{
			...
			{
				UE_LOG(LogBulletNetworkPrediction, Warning, TEXT("Actor %s: could not find a valid Primitive Component with Tag %s"), *Owner->GetPathName(), *NamedComponent.ToString());
			}
		}
