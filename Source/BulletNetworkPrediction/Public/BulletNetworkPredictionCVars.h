...
#ifndef BULLETNETSIM_CONST_CVARS
	#define BULLETNETSIM_CONST_CVARS (UE_BUILD_SHIPPING || UE_BUILD_TEST)
#endif
...
struct FBulletConditionalAutoConsoleRegister
{
	FBulletConditionalAutoConsoleRegister(const TCHAR* VarName, int32 Value, const TCHAR* Help)
	{
	...
	FBulletConditionalAutoConsoleRegister(const TCHAR* VarName, float Value, const TCHAR* Help)
	{
		if (!IConsoleManager::Get().FindConsoleVariable(VarName, false))
		{
			IConsoleManager::Get().RegisterConsoleVariable(VarName, Value, Help, ECVF_Cheat);
...
#define BULLETNETSIM_DEVCVAR_INT(Var,Value,VarName,Help) \
	static FBulletConditionalAutoConsoleRegister Var##Auto(TEXT(VarName),(int32)Value,TEXT(Help)); \
	inline int32 Var() \
	{ \
...
#if BULLETNETSIM_CONST_CVARS
#define BULLETNETSIM_DEVCVAR_SHIPCONST_INT(Var,Value,VarName,Help) \
	inline int32 Var() { return Value; } \
	inline void Set##Var(int32 _V) { }
#else
#define BULLETNETSIM_DEVCVAR_SHIPCONST_INT(Var,Value,VarName,Help) BULLETNETSIM_DEVCVAR_INT(Var,Value,VarName,Help)
#endif
...
#define BULLETNETSIM_DEVCVAR_FLOAT(Var,Value,VarName,Help) \
	static FBulletConditionalAutoConsoleRegister Var##Auto(TEXT(VarName),(float)Value,TEXT(Help)); \
	inline float Var() \
	{ \
		static const auto* Existing = FindConsoleVarHelper(TEXT(VarName)); \
		check(Existing); \
...
#if BULLETNETSIM_CONST_CVARS
#define BULLETNETSIM_DEVCVAR_SHIPCONST_FLOAT(Var,Value,VarName,Help) \
	inline float Var() { return Value; } \
	inline void Set##Var(float _V) { }
#else
#define BULLETNETSIM_DEVCVAR_SHIPCONST_FLOAT(Var,Value,VarName,Help) BULLETNETSIM_DEVCVAR_FLOAT(Var,Value,VarName,Help)
#endif
