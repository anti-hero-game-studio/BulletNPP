...
#define BULLETNETSIM_ENABLE_CHECKSUMS !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
#if BULLETNETSIM_ENABLE_CHECKSUMS 
#define BULLETNETSIM_CHECKSUM(Ser) SerializeChecksum(Ser,0xA186A384, false);
#else
#define BULLETNETSIM_CHECKSUM
#endif

#ifndef BULLETNETSIM_NETCONSTANT_NUM_BITS_FRAME
#define BULLETNETSIM_NETCONSTANT_NUM_BITS_FRAME 8	// Allows you to override this setting via UBT, but access via cleaner FActorMotionNetworkingConstants::NUM_BITS_FRAME
#endif
...
struct FBulletNetworkPredictionSerialization
{
	...
	enum { NUM_BITS_FRAME = BULLETNETSIM_NETCONSTANT_NUM_BITS_FRAME };
...
public:
	...
	// ------------------------------------------------------------------------------------------------------------
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletServerRecvData_Fixed<ModelDef>& ServerRecvData, TBulletModelDataStore<ModelDef>* DataStore, FBulletFixedTickState* TickState)
	{
		FArchive& Ar = P.Ar;
		int32 EndFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(Ar, ServerRecvData.LastRecvFrame); // 1. StartFrame
	...
	// ------------------------------------------------------------------------------------------------------------
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, FBulletFixedTickState* TickState)
	{
		FArchive& Ar = P.Ar;
		...
		for (int32 Frame = StartFrame; Frame < EndFrame; ++Frame)
		{
			...
			else
			{
				if (NetworkPredictionCVars::ForceSendDefaultInputCommands())
				{
					...
					FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(DefaultInputCmd, P); 
				}
				else
				{
					FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(Frames->Buffer[Frame].InputCmd, P); // 2. InputCmd
				}
			}
		}
	}
...
public:
	...
	// ------------------------------------------------------------------------------------------------------------
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletServerRecvData_Independent<ModelDef>& ServerRecvData, TBulletModelDataStore<ModelDef>* DataStore)
	{
		FArchive& Ar = P.Ar;
		

		const int32 EndFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(Ar, ServerRecvData.LastRecvFrame); // 1. StartFrame
		const int32 StartFrame = EndFrame - NumInputsPerSend;
		...
		for (int32 Frame=StartFrame; Frame < EndFrame; ++Frame)
		{
			...
			else
			{
				...
				typename TBulletServerRecvData_Independent<ModelDef>::FFrame& RecvFrame = ServerRecvData.InputBuffer[Frame];

				FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(RecvFrame.InputCmd, P); // 2. InputCmd
				FBulletNetworkPredictionSerialization::SerializeDeltaMS(P.Ar, RecvFrame.DeltaTimeMS); // 3. DeltaTime
	...
	// TIndependentTickReplicator_Server::NetRecv AP Client sending to server
	// ------------------------------------------------------------------------------------------------------------
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, FBulletVariableTickState* TickState)
	{
		FArchive& Ar = P.Ar;
		...
		for (int32 Frame = StartFrame; Frame < EndFrame; ++Frame)
		{
			...
			else
			{
				...
				FBulletNetworkPredictionSerialization::SerializeDeltaMS(P.Ar, TickState->Frames[Frame].DeltaMS); // 3. Delta InputCmd
			}
		}
	}
...
private:
	...
	static void EatCmd(const FBulletNetSerializeParams& P)
	{
		...
		int32 TimeMS = 0;
		FBulletNetworkPredictionSerialization::SerializeDeltaMS(P.Ar, TimeMS); // 3. Delta InputCmd
	}
...
public:
	...
	static void NetRecv(const FBulletNetSerializeParams& P, TInstanceData<ModelDef>& InstanceData, TBulletClientRecvData<ModelDef>& ClientRecvState, typename TBulletInstanceFrameState<ModelDef>::FFrame* BaseDeltaFrame)
	{
		// ** Modified By Kai Delta Serialization Support **//
		if(BaseDeltaFrame)
		{
			FBulletNetSerializeParams DeltaSerializationParams = P;
			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->SyncState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(ClientRecvState.SyncState, DeltaSerializationParams);	// 1. Sync
	...
	static void NetSend(const FBulletNetSerializeParams& P, TInstanceData<ModelDef>& InstanceData, typename TBulletInstanceFrameState<ModelDef>::FFrame& FrameData, typename TBulletInstanceFrameState<ModelDef>::FFrame* BaseDeltaFrame)
	{
		// ** Modified By Kai Delta Serialization Support **//
		if (BaseDeltaFrame)
		{
			FBulletNetSerializeParams DeltaSerializationParams = P;
			DeltaSerializationParams.BaseDeltaStatePtr = (void*)(BaseDeltaFrame->SyncState.Get());
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(FrameData.SyncState, DeltaSerializationParams);	// 1. Sync
			DeltaSerializationParams.BaseDeltaStatePtr = (void*) BaseDeltaFrame->AuxState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(FrameData.AuxState, DeltaSerializationParams);	// 2. Aux
		}
		// ** End Modified By Kai Delta Serialization Support **//
		else
		{
...
public:
	...
	// ------------------------------------------------------------------------------------------------------------
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletClientRecvData<ModelDef>& ClientRecvState, TBulletModelDataStore<ModelDef>* DataStore, FBulletFixedTickState* TickState)
	{
		FArchive& Ar = P.Ar;
		...
		if (HasAckedFrame)
		{
			DeltaStateFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(Ar, 0); // 2. acked delta number
		}
		...
		if (DeltaStateFrame != INDEX_NONE)
		{
			...
			if (!BaseDeltaFrame)
			{
				...
				UE_LOG(LogBulletNetworkPrediction,Warning,TEXT("Client ID %d Discarded Update from Missing Delta State %d"),ClientRecvState.ID,DeltaStateFrame)
				return;
			}
		...
		const int32 LastConsumedInputFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(Ar, TickState->PendingFrame); // 4. Last Consumed (Client) Input Frame
		const int32 ServerFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(Ar, TickState->PendingFrame + TickState->Offset); // 5. Server's Frame
		...
		{
			typename TBulletInstanceFrameState<ModelDef>::FFrame& AckedFrameData = ClientRecvState.AckedFrames.FindOrAdd(ServerFrame);
		...
		InstanceData.CueDispatcher->NetRecvSavedCues(P.Ar, true, ServerFrame, 0); // 7. BulletNetSimCues
	}
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, const FBulletFixedTickState* TickState)
	{
		...
		if (UBulletNetworkPredictionPlayerControllerComponent* RPCHandler = Instance->Info.RPCHandler)
		{
		...
		{
			if (const FBulletAckedFrames* NetConnectionAckedFrames = TickState->ServerAckedFrames.ConnectionsAckedFrames.Find(NetConnection))
			{
		...
		if (HasAckedFrame)
		{
			FBulletNetworkPredictionSerialization::WriteCompressedFrame(Ar, AckedFrame);// 2. Delta frame number
		}
		...
		FBulletNetSerializeParams LocalParams(LocalNetWriter,P.Map,P.ReplicationTarget);

		//-----------------------------------------------------------------------------------//
		
		FBulletNetworkPredictionSerialization::WriteCompressedFrame(LocalParams.Ar, LastConsumedFrame); // 4. Last Consumed Input Frame (Client's frame)
		FBulletNetworkPredictionSerialization::WriteCompressedFrame(LocalParams.Ar, PendingFrame); // 5. PendingFrame (Server's frame)

		TCommonReplicator_AP<ModelDef>::NetSend(LocalParams, *Instance, Frames->Buffer[PendingFrame],BaseDeltaFrame); // 6. Common

		Instance->CueDispatcher->NetSendSavedCues(LocalParams.Ar, EBulletNetSimCueReplicationTarget::AutoProxy, true); // 7. BulletNetSimCues
...
public:
	...
	// AP Client receives from the server
	// ------------------------------------------------------------------------------------------------------------
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletClientRecvData<ModelDef>& ClientRecvState, TBulletModelDataStore<ModelDef>* DataStore, FBulletVariableTickState* TickState)
	{
		FArchive& Ar = P.Ar;
	...
	// Server sends to AP client
	// ------------------------------------------------------------------------------------------------------------
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, TBulletServerRecvData_Independent<ModelDef>& ServerRecvData, const FBulletVariableTickState* VariableTickState)
	{
		FArchive& Ar = P.Ar;
		FBulletNetworkPredictionSerialization::WriteCompressedFrame(Ar, ServerRecvData.LastConsumedFrame); // 1. Last Consumed Input Frame (Client's frame)
		FBulletNetworkPredictionSerialization::SerializeTimeMS(Ar, ServerRecvData.TotalSimTimeMS); // 2. TotalSimTime

		TInstanceData<ModelDef>& InstanceData = DataStore->Instances.GetByIndexChecked(ServerRecvData.InstanceIdx);
		TBulletInstanceFrameState<ModelDef>& Frames = DataStore->Frames.GetByIndexChecked(ServerRecvData.FramesIdx);
...
//	2. Independent Tick, remotely controlled: send total sim time, which comes from the server's TBulletServerRecvData_Independent for the controlling client.
...
public:
	...
	static void NetRecv(const FBulletNetSerializeParams& P, ClientRecvDataType& ClientRecvState, TBulletModelDataStore<ModelDef>* DataStore, typename TBulletInstanceFrameState<ModelDef>::FFrame* BaseDeltaFrame)
	{
		// ** Modified By Kai Delta Serialization Support **//
		if (BaseDeltaFrame)
		{
			...
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(ClientRecvState.InputCmd, DeltaSerializationParams);	// 1. Input

			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->SyncState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(ClientRecvState.SyncState, DeltaSerializationParams);	// 2. Sync

			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->AuxState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(ClientRecvState.AuxState, DeltaSerializationParams);	// 3. Aux
		}
		// ** End Modified By Kai Delta Serialization Support **//
		else
		{
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, TInstanceData<ModelDef>* InstanceData, int32 PendingFrame, typename TBulletInstanceFrameState<ModelDef>::FFrame* BaseDeltaFrame)
	{
		...
		if (BaseDeltaFrame)
		{
			FBulletNetSerializeParams DeltaSerializationParams = P;
			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->InputCmd.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(FrameData.InputCmd, DeltaSerializationParams);	// 1. Input

			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->SyncState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(FrameData.SyncState, DeltaSerializationParams);	// 2. Sync

			DeltaSerializationParams.BaseDeltaStatePtr = (void*)BaseDeltaFrame->AuxState.Get();
			FBulletNetworkPredictionDriver<ModelDef>::NetSerialize(FrameData.AuxState, DeltaSerializationParams);	// 3. Aux
		}
		// ** End Modified By Kai Delta Serialization Support **//
		else
		{
...
public:
	...
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletClientRecvData<ModelDef>& ClientRecvState, TBulletModelDataStore<ModelDef>* DataStore, FBulletFixedTickState* TickState)
	{
		...
		if (HasAckedFrame)
		{
			DeltaStateFrame = FBulletNetworkPredictionSerialization::ReadCompressedFrame(P.Ar, 0); // 2. Delta Frame Num
		}
		...
		if (ClientRecvState.ServerFrame > 0)
		{
			...
			typename TBulletInstanceFrameState<ModelDef>::FFrame& AckedFrameData = ClientRecvState.AckedFrames.FindOrAdd(AckedFrame);
			ClientRecvState.SyncState.CopyTo(AckedFrameData.SyncState);
			ClientRecvState.AuxState.CopyTo(AckedFrameData.AuxState);
		...
		InstanceData.CueDispatcher->NetRecvSavedCues(P.Ar, bSerializeCueFrames, ClientRecvState.ServerFrame, 0); // 6. BulletNetSimCues
	}
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, const FBulletFixedTickState* TickState)
	{
		...
		typename TBulletInstanceFrameState<ModelDef>::FFrame* BaseDeltaFrame = nullptr;
		int32 AckedFrame = INDEX_NONE;
		if (const FBulletAckedFrames* AckedFrames = TickState->ServerAckedFrames.ConnectionsAckedFrames.Find(NetConnection))
		{
		...
		{
			BaseDeltaFrame = &Frames->Buffer[AckedFrame]; 
			FBulletNetworkPredictionSerialization::WriteCompressedFrame(P.Ar, AckedFrame); // 2. Delta Frame Num
		}
		...
		FBulletNetworkPredictionSerialization::WriteCompressedFrame(LocalParams.Ar, PendingFrame); // 4. PendingFrame (Server's frame)
		
		TCommonReplicator_SP<ModelDef>::NetSend(LocalParams, ID, DataStore, Instance, PendingFrame,BaseDeltaFrame); // 5. Common

		const bool bSerializeCueFrames = true; // Fixed tick can use Frame numbers for SP serialization
		Instance->CueDispatcher->NetSendSavedCues(LocalParams.Ar, EBulletNetSimCueReplicationTarget::SimulatedProxy | EBulletNetSimCueReplicationTarget::Interpolators, bSerializeCueFrames); // 6. BulletNetSimCues
...
public:
	...
	static void NetRecv(const FBulletNetSerializeParams& P, TBulletClientRecvData<ModelDef>& ClientRecvState, TBulletModelDataStore<ModelDef>* DataStore, FBulletVariableTickState* TickState)
	{
		...
		const bool bSerializeCueFrames = true; // Fixed tick can use Frame numbers for SP serialization
		InstanceData.CueDispatcher->NetRecvSavedCues(P.Ar, bSerializeCueFrames, INDEX_NONE, ClientRecvState.SimTimeMS); // 4. BulletNetSimCues
	}
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, const FBulletVariableTickState* TickState)
	{
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, const TBulletServerRecvData_Independent<ModelDef>& IndependentTickState, const FBulletVariableTickState* VariableTickState)
	{
...
private:
	...
	static void NetSend(const FBulletNetSerializeParams& P, FBulletNetworkPredictionID ID, TBulletModelDataStore<ModelDef>* DataStore, int32 IndependentSimTime, int32 ServerTotalSimTime, int32 PendingFrame)
	{
		...
		Instance->CueDispatcher->NetSendSavedCues(P.Ar, EBulletNetSimCueReplicationTarget::SimulatedProxy | EBulletNetSimCueReplicationTarget::Interpolators, bSerializeCueFrames); // 4. BulletNetSimCues
	}
};
