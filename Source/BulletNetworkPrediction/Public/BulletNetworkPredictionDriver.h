...
#include "BulletNetworkPredictionSettings.h"
#include "BulletNetworkPredictionDebug.h"
...
// This driver class is defined in the ModelDef (your version of FBulletNetworkPredictionModelDef). For example, AActor or AMyPawn class.
...
//
// FBulletNetworkPredictionDriver is how calls to the driver are made. FBulletNetworkPredictionDriverBase is a default implementation that will be used.
// You can customize any of these calls by specializing FBulletNetworkPredictionDriver to your ModelDef type. If you do this, you should inherit from 
// FBulletNetworkPredictionDriverBase<YourModelDef> or FBulletNetworkPredictionDriver<ParentModelDef> if you are extending an existing ModelDef.
...
//		FBulletNetworkPredictionDriver<YourModelDef>::InitializeSimulationState(AActor*, FSyncState*, FAuxState*) and implement it there.
//
...
template<typename ModelDef>
struct FBulletNetworkPredictionDriverBase
{
	...
	static constexpr FBulletNetworkPredictionModelDefCapabilities GetCapabilities()
	{
		FBulletNetworkPredictionModelDefCapabilities Capabilities;

		if (HasSimulation() == false)
		{
			// We have nothing to tick, so no SimExtrapolation or ForwardPrediction
			Capabilities.FixedNetworkLODs.AP = EBulletNetworkLOD::Interpolated;
			Capabilities.FixedNetworkLODs.SP = EBulletNetworkLOD::Interpolated;
			Capabilities.IndependentNetworkLODs.AP = EBulletNetworkLOD::Interpolated;
			Capabilities.IndependentNetworkLODs.SP = EBulletNetworkLOD::Interpolated;
		}
		return Capabilities;
	}

	// Defines the default settings for a spawn instance.
	static bool GetDefaultArchetype(FBulletNetworkPredictionInstanceArchetype& Archetype, EBulletNetworkPredictionTickingPolicy PreferredTickingPolicy)
	{
	...
	static FBulletNetworkPredictionInstanceConfig GetConfig(const FBulletNetworkPredictionInstanceArchetype& Archetype, const FBulletNetworkPredictionSettings& GlobalSettings, ENetRole Role, bool bHasNetConnection)
	{
		static constexpr FBulletNetworkPredictionModelDefCapabilities Capabilities = FBulletNetworkPredictionDriver<ModelDef>::GetCapabilities();
		FBulletNetworkPredictionInstanceConfig Config;
		switch (Role)
		{
		case ROLE_Authority:
			Config.InputPolicy = bHasNetConnection ? EBulletNetworkPredictionLocalInputPolicy::Passive : EBulletNetworkPredictionLocalInputPolicy::PollPerSimFrame;
			break;
		case ROLE_AutonomousProxy:
			Config.InputPolicy = EBulletNetworkPredictionLocalInputPolicy::PollPerSimFrame;
			Config.NetworkLOD = Archetype.TickingMode == EBulletNetworkPredictionTickingPolicy::Fixed ? GetHighestNetworkLOD(Capabilities.FixedNetworkLODs.AP) : GetHighestNetworkLOD(Capabilities.IndependentNetworkLODs.AP);
			break;
		case ROLE_SimulatedProxy:
			Config.InputPolicy = EBulletNetworkPredictionLocalInputPolicy::Passive;

			// Use preferred SP NetworkLOD if we support it
			const EBulletNetworkLOD CapableLODs = (Archetype.TickingMode == EBulletNetworkPredictionTickingPolicy::Fixed) ? Capabilities.FixedNetworkLODs.SP : Capabilities.IndependentNetworkLODs.SP;
	...
	{
		BulletNetworkPredictionDebug::DrawDebugOutline(GetDebugWorldTransform(Driver), GetDebugBoundingBox(Driver), Color, Lifetime);
	}
	...
	static void DrawDebugText3D(DriverType* Driver, const TCHAR* Str, FColor Color, float Lifetime=0.f, float ZOffset=50.f)
	{
		...
		BulletNetworkPredictionDebug::DrawDebugText3D(Str, Transform , Color, Lifetime);
	}
	...
	static void GetSmoothingStateDelta(DriverType* Driver,  const SyncType* CurrentSyncState, const AuxType* CurrentAuxState,const SyncType* PrevSyncState, const AuxType* PrevAuxState,  SyncType* DeltaSyncState,  AuxType* DeltaAuxState)
	{
		...
		{
			FBulletNetworkPredictionDriver<ModelDef>::CallGetSmoothingStateDeltaMemberFunc(Driver, CurrentSyncState, CurrentAuxState, PrevSyncState,PrevAuxState,DeltaSyncState,DeltaAuxState);
		}
	}
	...
	static void GetSmoothingStateScaled(DriverType* Driver, const SyncType* SyncState, const AuxType* AuxState, const float Scale,  SyncType* ScaledSyncState,  AuxType* ScaledAuxState)
	{
		if (IsValidObject(Driver))
		{
			FBulletNetworkPredictionDriver<ModelDef>::CallGetSmoothingStateScaledMemberFunc(Driver, SyncState,AuxState, Scale,ScaledSyncState,ScaledAuxState);
		}
	}
	...
	static void GetSmoothingStateUnion(DriverType* Driver, const SyncType* CurrentSyncState, const AuxType* CurrentAuxState,const SyncType* DeltaSyncState, const AuxType* DeltaAuxState,  SyncType* UnionSyncState,  AuxType* UnionAuxState)
	{
		if (IsValidObject(Driver))
		{
			FBulletNetworkPredictionDriver<ModelDef>::CallGetSmoothingStateUnionMemberFunc(Driver, CurrentSyncState, CurrentAuxState, DeltaSyncState, DeltaAuxState, UnionSyncState, UnionAuxState);
		}
	}
	...
	{
		FBulletNetworkPredictionDriver<ModelDef>::CallRestoreFrameMemberFunc(Driver, SyncState, AuxState);
	}
	...
	//	CallServerRPC
	//
	//	Tells the driver to call the Server RPC to send InputCmds to the server. UBulletNetworkPredictionComponent::CallServerRPC is the default
	...
	static void DispatchCues(TBulletNetSimCueDispatcher<ModelDef>* CueDispatcher, void* Driver, int32 SimFrame, int32 SimTimeMS, const int32 FixedStepMS)
	{
	}
	...
	// -----------------------------------------------------------------------------------------------------------------------------------
	static bool ShouldReconcile(const TBulletSyncAuxPair<StateTypes>& Predicted, const TBulletSyncAuxPair<StateTypes>& Authority)
	{
	...
	// -----------------------------------------------------------------------------------------------------------------------------------
	static void Interpolate(const TBulletSyncAuxPair<StateTypes>& From, const TBulletSyncAuxPair<StateTypes>& To, const float PCT, SyncType* SyncOut, AuxType* AuxOut)
	{
	...
	// -----------------------------------------------------------------------------------------------------------------------------------
	static void LogUserStates(const TBulletNetworkPredictionState<StateTypes>& UserStates, FOutputDevice& Ar=*GLog)
	{
	...
	// -----------------------------------------------------------------------------------------------------------------------------------
	template<typename StateType>
	static void NetSerialize(TBulletConditionalState<StateType>& State, const FBulletNetSerializeParams& P)
	{
		State->NetSerialize(P);
	}

	static void NetSerialize(TBulletConditionalState<void>& State, const FBulletNetSerializeParams& P) { }
};


// This is the actual template to specialize when wanting to override functions
template<typename ModelDef>
struct FBulletNetworkPredictionDriver : FBulletNetworkPredictionDriverBase<ModelDef>
{
