...
// Helper struct to encapsulate optional, delayed writing of new element to TBulletNetworkSimAuxBuffer buffer
template<typename ElementType>
struct TBulletNetSimLazyWriterFunc
{
	template<typename TBuffer>
	TBulletNetSimLazyWriterFunc(TBuffer& Buffer, int32 PendingFrame)
	{
		GetFunc = [&Buffer, PendingFrame]() { return Buffer.WriteAtFrame(PendingFrame); };
	}

	// Hack
	TBulletNetSimLazyWriterFunc(void* Data)
	{
...
// Reference version of LazyWriter. This is what gets passed through chains of ::SimulationTick calls. This is to avoid copying the TFunction in TBulletNetSimLazyWriterFunc around.
template<typename ElementType>
struct TBulletNetSimLazyWriter
{
	template<typename TLazyWriter>
	TBulletNetSimLazyWriter(const TLazyWriter& Parent)
...
template<typename ElementType> 
struct TBulletNetworkSimAuxBuffer
{
	TBulletNetworkSimAuxBuffer(int32 Capacity)
	{
		Init(Capacity);
	}
	...
	TBulletNetSimLazyWriterFunc<ElementType> LazyWriter(int32 Frame)
	{
		return TBulletNetSimLazyWriterFunc<ElementType>(*this, Frame);
	}
...
struct TBulletNetworkPredictionBuffer
{
	TBulletNetworkPredictionBuffer(int32 Capacity=2)
	{
		check(Capacity > 0);
		Init(Capacity);
	}

	TBulletNetworkPredictionBuffer(const TBulletNetworkPredictionBuffer<ElementType, SingleElement>&& Other)
		: IndexMask(Other.IndexMask)
		, Elements(MoveTemp(Other.Elements))
	{
	}

	TBulletNetworkPredictionBuffer<ElementType, SingleElement>& operator=(TBulletNetworkPredictionBuffer<ElementType, SingleElement>&& Other)
	{
		IndexMask = Other.IndexMask;
		Elements = MoveTemp(Other.Elements);
		return *this;
...
template<typename ElementType> 
struct TBulletNetworkPredictionBuffer<ElementType, true>
{
