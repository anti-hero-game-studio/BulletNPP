...
struct TBulletNetworkedSimulationState;

class BULLETNETWORKPREDICTION_API FBulletNetworkPredictionTrace
{
public:

	template<typename ModelDef>
	static void TraceSimulationCreated(FBulletNetworkPredictionID ID, typename ModelDef::Driver* Driver)
	{
		TStringBuilder<256> Builder;
		FBulletNetworkPredictionDriver<ModelDef>::GetTraceString(Driver, Builder);
		TraceSimulationCreated_Internal(ID, Builder);
	}

	static void TraceWorldFrameStart(UGameInstance* GameInstance, float DeltaSeconds);
	static void TraceSimulationConfig(int32 TraceID, ENetRole NetRole, bool bHasNetConnection, const FBulletNetworkPredictionInstanceArchetype& Archetype, const FBulletNetworkPredictionInstanceConfig& Config, int32 ServiceMask);
...
private:
	...
	static void TraceSimulationCreated_Internal(FBulletNetworkPredictionID ID, FStringBuilderBase& Builder);
