...
#include "BulletNetworkPredictionCueTraits.h"
...
-They are dispatched *after* the simulation is done ticking (via TBulletNetSimCueDispatcher::DispatchCueRecord which is called during the owning component's tick).
...
Notes on "simulation affecting events" (E.g, *not* BulletNetSimCues)
...
BULLETNETWORKPREDICTION_API DECLARE_LOG_CATEGORY_EXTERN(LogBulletNetworkPredictionCues, Display, All);

#ifndef BULLETNETSIMCUE_TYPEID_TYPE
#define BULLETNETSIMCUE_TYPEID_TYPE uint8;
#endif

using FBulletNetSimCueTypeId = BULLETNETSIMCUE_TYPEID_TYPE;

template<typename ModelDef>
struct FBulletNetworkPredictionDriver;

// ----------------------------------------------------------------------------------------------------------------------------------------------
//	Wrapper: wraps the actual user BulletNetSimCue. We want to avoid virtualizing functions on the actual user types.
// ----------------------------------------------------------------------------------------------------------------------------------------------

struct FBulletNetSimCueWrapperBase
{
	virtual ~FBulletNetSimCueWrapperBase() { }
	...
	virtual EBulletNetSimCueReplicationTarget GetReplicationTarget() const = 0;
};

template<typename TCue>
struct TBulletNetSimCueWrapper : FBulletNetSimCueWrapperBase
{
	TBulletNetSimCueWrapper() = default;

	template <typename... ArgsType>
	TBulletNetSimCueWrapper(ArgsType&&... Args)
		: Instance( MoveTempIfPossible(Forward<ArgsType>(Args))... ) { }

	void NetSerialize(FArchive& Ar) override final
	{
		TBulletNetCueNetSerializeHelper<TCue>::CallNetSerializeOrNot(Instance, Ar);
	}

	bool NetIdentical(const void* OtherCueData) const override final
	{
		return TBulletNetCueNetIdenticalHelper<TCue>::CallNetIdenticalOrNot(Instance, *((const TCue*)OtherCueData));
	}
	...
	EBulletNetSimCueReplicationTarget GetReplicationTarget() const override final
	{
		return TBulletNetSimCueTraits<TCue>::ReplicationTarget;
	}
...
struct FBulletNetSimCueCallbacks
{
...
/** System parameters for BulletNetSimCue events */
struct FBulletNetSimCueSystemParamemters
{
	...
	FBulletNetSimCueCallbacks* Callbacks;	
};
...
public:
	...
	FORCENOINLINE static void RegisterCueType(const FString& TypeName)
	{
		...
		UE_LOG(LogBulletNetworkPredictionCues, Verbose, TEXT("RegisterCueType %s @ 0x%X. (NumPending: %d) "), *TypeName, &TCue::ID, RegisteredTypes.CueTypes.Num());
		...
		{
			&TCue::ID, 
			[](){ return new TBulletNetSimCueWrapper<TCue>();},
			TypeName
		};
		RegisteredTypes.CueTypes.Emplace(TypeInfo);
		
		static_assert(THasMemberFunction_NetSerialize<TCue>::Value || !TBulletNetSimCueTypeRequirements<TCue>::RequiresNetSerialize, "TCue must implement void NetSerialize(FArchive&)");
		static_assert(TBulletHasNetIdenticalHelper<TCue>::Value || !TBulletNetSimCueTypeRequirements<TCue>::RequiresNetIdentical, "TCue must implement bool NetIdentical(const TCue&) const");
	}
	...
	static void UnregisterCueType()
	{
		...
		UE_LOG(LogBulletNetworkPredictionCues, Verbose, TEXT("UnregisterCueType 0x%X."), &TCue::ID);
	...
	void FinalizeCueTypes()
	{
		...
		{
			UE_LOG(LogBulletNetworkPredictionCues, Verbose, TEXT("FinalizeTypes: 0x%X. %d Pending Types"), &RegisteredTypes, RegisteredTypes.CueTypes.Num());
			...
			for (FTypeInfo& TypeInfo: RegisteredTypes.CueTypes)
			{
				...
				*TypeInfo.IDPtr = ++ID;
				...
				UE_LOG(LogBulletNetworkPredictionCues, Verbose, TEXT("    Cue %s assigned ID: %d (0x%X)"), *TypeInfo.TypeName, *TypeInfo.IDPtr, TypeInfo.IDPtr);
			}
	...
	BULLETNETWORKPREDICTION_API static FGlobalCueTypeTable& Get()
	{
		return Singleton;
	}

	FBulletNetSimCueWrapperBase* Allocate(FBulletNetSimCueTypeId ID)
	{
		return TypeInfoMap.FindChecked(ID).AllocateFunc();
	}

	FString GetTypeName(FBulletNetSimCueTypeId ID) const
	{
...
private:
	...
	struct FTypeInfo
	{
		FBulletNetSimCueTypeId* IDPtr = nullptr;
		TFunction<FBulletNetSimCueWrapperBase*()> AllocateFunc;
		FString TypeName;
	};
	...
	BULLETNETWORKPREDICTION_API static FRegisteredCueTypeInfo& GetRegistedTypeInfo();
	
	TMap<FBulletNetSimCueTypeId, FTypeInfo> TypeInfoMap;

	BULLETNETWORKPREDICTION_API static FGlobalCueTypeTable Singleton;
};

// ----------------------------------------------------------------------------------------------------------------------------------------------
// SavedCue: a recorded Invocation of a BulletNetSimCue
// ----------------------------------------------------------------------------------------------------------------------------------------------
...
struct FSavedCue
{
	...
	FSavedCue(const FBulletNetSimCueTypeId& InId, const int32& InFrame, const int32& InTime, const bool& bInAllowRollback, const bool& bInNetConfirmed, const bool& bInResimulates, FBulletNetSimCueWrapperBase* Cue)
	...
	FBulletNetSimCueTypeId ID = 0;
	int32 Frame = INDEX_NONE;
	int32 Time = 0;
	TUniquePtr<FBulletNetSimCueWrapperBase> CueInstance;
	FBulletNetSimCueCallbacks Callbacks;

	bool bDispatched = false;		// Cue has been dispatched to the local handler. Never dispatch twice.
	bool bAllowRollback = false;	// Cue supports rolling back. ie., we should pass the user valid FBulletNetSimCueCallbacks rollback callback (note this is "this saved cue" specifically, not "this cue type". E.g, on authority, bAllowRollback is always false)
	...
	EBulletNetSimCueReplicationTarget ReplicationTarget;
};
...
public:
	...
	void RegisterType()
	{
		...
		CueTypeInfo.Dispatch = [](FBulletNetSimCueWrapperBase* Cue, TCueHandler& Handler, const FBulletNetSimCueSystemParamemters& SystemParameters)
		{
	...
	void Dispatch(FSavedCue& SavedCue, TCueHandler& Handler, const int32& TimeSinceInvocation)
	{
		...
		else
		{
			...
			UE_LOG(LogBulletNetworkPredictionCues, Warning, TEXT("Could not Find NetCue %s (ID: %d) in dispatch table (%d entries)."), *FGlobalCueTypeTable::Get().GetTypeName(SavedCue.ID), SavedCue.ID, CueTypeInfoMap.Num());
			for (auto MapIt : CueTypeInfoMap)
			{
				UE_LOG(LogBulletNetworkPredictionCues, Warning, TEXT("  Have: %d"), MapIt.Key);
			}
		}
	}
...
private:
	...
	struct FCueTypeInfo
	{
		TFunction<void(FBulletNetSimCueWrapperBase* Cue, TCueHandler& Handler, const FBulletNetSimCueSystemParamemters& SystemParameters)> Dispatch;
	};

	TMap<FBulletNetSimCueTypeId, FCueTypeInfo> CueTypeInfoMap;
};
...
// Traits for TBulletNetSimCueDispatcher
struct BULLETNETWORKPREDICTION_API FCueDispatcherTraitsBase
{
	// Window for replicating a BulletNetSimCue. That is, after a cue is invoked, it has ReplicationWindow time before it will be pruned.
...
{
	virtual ~FBulletNetSimCueDispatcher() = default;
	...
	void Invoke(ArgsType&&... Args)
	{
		...
		{
			if (EnumHasAnyFlags(TBulletNetSimCueTraits<T>::SimTickMask(), Context.TickContext))
			{
				constexpr bool bSupportsResimulate = TBulletNetSimCueTraits<T>::Resimulate;
				...
				if (Context.TickContext == EBulletSimulationTickContext::Authority)
				{
					...
					bTransient = TBulletNetSimCueTraits<T>::ReplicationTarget == EBulletNetSimCueReplicationTarget::None;
					bNetConfirmed = true;
				}
				else
				{
					...
					bAllowRollback = (TBulletNetSimCueTraits<T>::ReplicationTarget == EBulletNetSimCueReplicationTarget::All) || bSupportsResimulate;
				...
				if (Context.TickContext == EBulletSimulationTickContext::Resimulate)
				{
					...
					// (not as nice as the non resimulate path, but better than allocating a new FSavedCue+TBulletNetSimCueWrapper and then removing them)
					...
					for (FSavedCue& ExistingCue : SavedCues)
					{
						...
						if (RollbackFrame <= ExistingCue.Frame && ExistingCue.NetIdentical(NewCue))
						{
							...
							UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Resimulated Cue %s matched existing cue. Suppressing Invocation. Time: %d"), *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), Context.CurrentSimTime);
							return;
						}
					}

					
					auto& SavedCue = GetBuffer(bTransient).Emplace_GetRef(T::ID, Context.Frame, Context.CurrentSimTime, bAllowRollback, bNetConfirmed, bSupportsResimulate, new TBulletNetSimCueWrapper<T>(MoveTempIfPossible(NewCue)));
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Invoking Cue %s during resimulate. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d"), *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}
				else
				{
					// Not resimulate case is simple: construct the new cue emplace in the appropriate list	
					auto& SavedCue = GetBuffer(bTransient).Emplace_GetRef(T::ID, Context.Frame, Context.CurrentSimTime, bAllowRollback, bNetConfirmed, bSupportsResimulate, new TBulletNetSimCueWrapper<T>(Forward<ArgsType>(Args)...));
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Invoking Cue %s. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d"), *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}				
			}
			else
			{
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s .Suppressing Cue Invocation %s. Mask: %d. TickContext: %d. Time: %d"), *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), int(TBulletNetSimCueTraits<T>::SimTickMask()), (int32)Context.TickContext, Context.CurrentSimTime);
			}
		}
	}
...
protected:
	...
	struct FContext
	{
		...
		EBulletSimulationTickContext TickContext;
	};
...
struct TBulletNetSimCueDispatcher : public FBulletNetSimCueDispatcher
{
	...
	void NetSendSavedCues(FArchive& Ar, EBulletNetSimCueReplicationTarget ReplicationMask, bool bSerializeFrameNumber)
	{
		...
		FBulletNetSimCueTypeId NumCues = 0;
	...
	void NetRecvSavedCues(FArchive& Ar, const bool bSerializeFrameNumber, const int32 InLastRecvFrame, const int32 InLastRecvTime)
	{
		...
		for (int32 CueIdx=0; CueIdx < NumCues; ++CueIdx)
		{
			...
			{
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Discarding replicated BulletNetSimCue %s that is not intended for us. CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			if (SerializedCue.Frame != INDEX_NONE)
			{
				...
				{
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Discarding replicated BulletNetSimCue %s because it is older (%d) than confirmed frame (%d). CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
					continue;
				}
			}
			else if (SerializedCue.Time <= LastRecvMS)
			{
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Discarding replicated BulletNetSimCue %s because it is older (%d) than confirmed TimeMS (%d). CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			for (int32 ExistingIdx=0; ExistingIdx < StartingNum; ++ExistingIdx)
			{
				...
				if (SerializedCue.NetIdentical(ExistingCue))
				{
					...
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Discarding replicated BulletNetSimCue %s because we've already processed it. (Matched %s)"), *GetDebugName(), *SerializedCue.GetDebugName(), *ExistingCue.GetDebugName());
					bUniqueCue = false;
			...
			if (bUniqueCue)
			{
				...
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Received !NetIdentical Cue: %s. (Num replicated cue sent this bunch: %d. LastRecvFrame: %d. Our Mask: %d)."), *GetDebugName(), *SerializedCue.GetDebugName(), NumCues, LastRecvFrame, int(RecvReplicationMask));
			}
		}
	...
	void DispatchCueRecord(T& Handler, const int32 SimFrame, const int32 CurrentSimTime, const int32 FixedTimeStepMS)
	{
		...
		if (RollbackFrame >= 0)
		{
			for (auto It = SavedCues.CreateIterator(); It; ++It)
			{
				...
				if (SavedCue.bPendingResimulateRollback)
				{
					...
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Calling OnRollback for SavedCue BulletNetSimCue %s. Cue was not matched during a resimulate."), *GetDebugName(), *SavedCue.GetDebugName());
					SavedCue.Callbacks.OnRollback.Broadcast();
					SavedCues.RemoveAt(It.GetIndex(), EAllowShrinking::No);
				}
			}
		...
		for (int32 SavedCueIdx = 0; SavedCueIdx < SavedCues.Num(); ++ SavedCueIdx)
		{
			...
			if (!SavedCue.bDispatched)
			{
				...
				if (bDispatchedCue)
				{
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s Dispatching BulletNetSimCue %s {Frame: %d, Time: %d}"), *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time);
					SavedCue.bDispatched = true;
					TCueDispatchTable<T>::Get().Dispatch(SavedCue, Handler, TimeSinceInvocationMS);
				}
				else
				{
					UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s Withholding Cue %s. %d/%d > %d/%d"), *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time, SimFrame, CurrentSimTime);
				}
			}
		}
		...
		{
			UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("Dispatching transient BulletNetSimCue %s."), *TransientCue.GetDebugName());
		...
		if (SavedCuePruneIdx >= 0)
		{
			...
			{
				UE_CLOG(!SavedCues[i].bDispatched, LogBulletNetworkPredictionCues, Warning, TEXT("Non-Dispatched Cue is about to be pruned! %s. SavedCuePruneTimeMS: %d. SavedCuePruneFrame: %d"), *SavedCues[i].GetDebugName(), SavedCuePruneTimeMS, SavedCuePruneFrame);
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Pruning Cue %s. Prune: (%d/%d). Invoked: (%d/%d). Current Time: %d."), *GetDebugName(), *SavedCues[i].GetDebugName(), SavedCuePruneFrame, SavedCuePruneTimeMS,  SavedCues[i].Frame, SavedCues[i].Time, CurrentSimTime);
			}
#endif
	...
	void NotifyRollback(int32 InRollbackFrame)
	{
		...
		for (FSavedCue& SavedCue : SavedCues)
		{
			...
			if (SavedCue.bResimulates && SavedCue.Frame >= InRollbackFrame)
			{
				...
				UE_LOG(LogBulletNetworkPredictionCues, Log, TEXT("%s. Marking %s bPendingResimulateRollback. (RollbackFrame: %d/%d)"), *GetDebugName(), *SavedCue.GetDebugName(), RollbackFrame, InRollbackFrame);
			}
		}
	}

	// Set which cues we should accept locally (if they get sent to us)
	void SetReceiveReplicationTarget(EBulletNetSimCueReplicationTarget InReplicationMask)
	{
	...
	FString GetDebugName() const final override
	{
		...
		if (Driver)
		{
			...
			FBulletNetworkPredictionDriver<ModelDef>::GetDebugString(Driver, Builder);
...
private:
	...
	EBulletNetSimCueReplicationTarget RecvReplicationMask;
...
// Body of BulletNetSimCue header. Gives type a static ID to be identified with
#define BULLETNETSIMCUE_BODY() static FBulletNetSimCueTypeId ID

// ------------------------------------------------------------------------------------------------
// Register a cue type with the global table.
// ------------------------------------------------------------------------------------------------
template<typename TCue>
struct TBulletNetSimCueTypeAutoRegisterHelper
{
	TBulletNetSimCueTypeAutoRegisterHelper(const FString& Name)
	{
		FGlobalCueTypeTable::RegisterCueType<TCue>(Name);
	}

	~TBulletNetSimCueTypeAutoRegisterHelper()
	{
...
#define BULLETNETSIMCUE_REGISTER(X, STR) FBulletNetSimCueTypeId X::ID=0; TBulletNetSimCueTypeAutoRegisterHelper<X> BulletNetSimCueAr_##X(STR);

// ------------------------------------------------------------------------------------------------
// Register a handler's cue types via a static "RegisterBulletNetSimCueTypes" on the handler itself
// ------------------------------------------------------------------------------------------------
template<typename THandler>
struct TBulletNetSimCueHandlerAutoRegisterHelper
{
	TBulletNetSimCueHandlerAutoRegisterHelper()
	{
		Handle = FGlobalCueTypeTable::RegisterDispatchTableCallback([]()
		{
			TCueDispatchTable<THandler>::Get().Reset();
			THandler::RegisterBulletNetSimCueTypes( TCueDispatchTable<THandler>::Get() );
		});
	}
...
#define BULLETNETSIMCUEHANDLER_REGISTER(X) TBulletNetSimCueHandlerAutoRegisterHelper<X> BulletNetSimCueHandlerAr_##X;

// ------------------------------------------------------------------------------------------------
// Register a handler's cue types via an intermediate "set" class with a static "RegisterBulletNetSimCueTypes" function
// ------------------------------------------------------------------------------------------------
template<typename THandler, typename TCueSet>
struct TBulletNetSimCueSetHandlerAutoRegisterHelper
{
	TBulletNetSimCueSetHandlerAutoRegisterHelper()
	{
		Handle = FGlobalCueTypeTable::RegisterDispatchTableCallback([]()
		{
			TCueDispatchTable<THandler>::Get().Reset();
			TCueSet::RegisterBulletNetSimCueTypes( TCueDispatchTable<THandler>::Get() );
		});
	}
...
#define BULLETNETSIMCUESET_REGISTER(THandler, TCueSet) TBulletNetSimCueSetHandlerAutoRegisterHelper<THandler,TCueSet> BulletNetSimCueSetHandlerAr_##THandler_##TCueSet;
