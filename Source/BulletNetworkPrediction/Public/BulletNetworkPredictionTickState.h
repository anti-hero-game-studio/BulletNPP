...
#include "BulletNetworkPredictionDeltaSerializationData.h"


// TimeStep info that is passed into SimulationTick
struct FBulletNetSimTimeStep
{
	...
	FBulletNetSimTimeStep(int32 InStepMS, int32 InTotalSimulationTime, int32 InFrame)
		: StepMS(InStepMS)
...
struct FBulletFixedTickState
{
	...
	FBulletNetSimTimeStep GetNextTimeStep() const
	{
		return FBulletNetSimTimeStep{FixedStepMS, GetTotalSimTimeMS(),  PendingFrame+1 + Offset};
	}

	FBulletServiceTimeStep GetNextServiceTimeStep() const
	{
		return FBulletServiceTimeStep{PendingFrame, PendingFrame+1,(PendingFrame+Offset+1) * FixedStepMS};
	}
...
struct FBulletVariableTickState
{
	struct FFrame
	{
		int32 DeltaMS=0;
	...
	TBulletNetworkPredictionBuffer<FFrame> Frames;

	FBulletVariableTickState() : Frames(64) { }
	...
	FBulletNetSimTimeStep GetNextTimeStep(const FFrame& PendingFrameData) const
	{
		return FBulletNetSimTimeStep{PendingFrameData.DeltaMS, PendingFrameData.TotalMS, PendingFrame+1};
	}

	FBulletServiceTimeStep GetNextServiceTimeStep(const FFrame& PendingFrameData) const
	{
		return FBulletServiceTimeStep{PendingFrame, PendingFrame+1, PendingFrameData.TotalMS + PendingFrameData.DeltaMS};
	}
