...
// Generic associative container for instance data keyed off of the FBulletNetworkPredictionID
...
template<typename T>
struct TBulletInstanceMap
{
	T& FindOrAdd(const FBulletNetworkPredictionID& ID)
	{
		return Data[GetIndex(ID)];
	}

	T* Find(const FBulletNetworkPredictionID& ID)
	{
	...
	void Remove(const FBulletNetworkPredictionID& ID)
	{
		FIndex RemovedIdx;
	...
	int32 GetIndexChecked(const FBulletNetworkPredictionID& ID)
	{
		return Lookup.FindChecked((int32)ID).idx;
	}

	int32 GetIndex(const FBulletNetworkPredictionID& ID)
	{
...
template<typename T>
struct TBulletStableInstanceMap
{
	T& FindOrAdd(const FBulletNetworkPredictionID& ID)
	{
