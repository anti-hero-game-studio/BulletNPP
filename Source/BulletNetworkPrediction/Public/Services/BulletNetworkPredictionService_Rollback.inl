...
{
	BULLETNETSIM_DEVCVAR_SHIPCONST_INT(ForceReconcile,			0, "jnp.ForceReconcile",				"Force a single reconcile back to the last server-acknoledged frame. When used with np.ForceReconcileExtraFrames, additional frames can be rolled back. No effect on server. Resets after use.");
	BULLETNETSIM_DEVCVAR_SHIPCONST_INT(ForceReconcileExtraFrames, 0, "jnp.ForceReconcileExtraFrames",	"Roll back this extra number of frames during the next targeted reconcile. Must be positive and reasonable given the buffer sizes.");
	BULLETNETSIM_DEVCVAR_SHIPCONST_INT(SkipReconcile,				0, "jnp.SkipReconcile",				"Skip all reconciles");
	BULLETNETSIM_DEVCVAR_SHIPCONST_INT(PrintReconciles,			0, "jnp.PrintReconciles",			"Print reconciles to log");
}
