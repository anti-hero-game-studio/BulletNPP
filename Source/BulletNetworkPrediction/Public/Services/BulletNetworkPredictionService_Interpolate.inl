...
// ------------------------------------------------------------------------------
class IBulletFixedInterpolateService
{
public:

	virtual ~IBulletFixedInterpolateService() = default;
	virtual void Reconcile(FBulletFixedTickState* TickState) = 0;
	virtual void FinalizeFrame(float DeltaTimeSeconds, const FBulletFixedTickState* TickState) = 0;
};

template<typename InModelDef>
class TBulletFixedInterpolateService : public IBulletFixedInterpolateService
{
public:
	...
	TBulletFixedInterpolateService(TBulletModelDataStore<ModelDef>* InDataStore)
	...
	void UnregisterInstance(FBulletNetworkPredictionID ID)
	{
		...
		const TBulletClientRecvData<ModelDef>& ClientRecvData = DataStore->ClientRecv.GetByIndexChecked(ClientRecvIdx);
		...
		if (InterpolationData.bTwoValidFrames == false)
		{
			...
			FBulletNetworkPredictionDriver<ModelDef>::SetHiddenForInterpolation(InstanceData.Info.Driver, false);
		}
	...
	void Reconcile(FBulletFixedTickState* TickState) final override
	{
		...
		for (TConstDualSetBitIterator<FDefaultBitArrayAllocator,FDefaultBitArrayAllocator> BitIt(ClientRecvBitMask, DataStore->ClientRecvBitMask); BitIt; ++BitIt)
		{
			...
			if (InterpolationData.bTwoValidFrames == false)
			{
				...
				{
					TInstanceData<ModelDef>& InstanceData = DataStore->Instances.GetByIndexChecked(InterpolationData.InstanceIdx);
					FBulletNetworkPredictionDriver<ModelDef>::SetHiddenForInterpolation(InstanceData.Info.Driver, true);
				}
			}
	...
	void FinalizeFrame(float DeltaTimeSeconds, const FBulletFixedTickState* TickState) final override
	{
		const int32 FromFrame = TickState->Interpolation.ToFrame-1;
		const int32 ToFrame = TickState->Interpolation.ToFrame;
		...
		if ( NetworkPredictionCVars::DisableInterpolation() > 0)
		{
			for (TConstSetBitIterator<> BitIt(ClientRecvBitMask); BitIt; ++BitIt)
			{
				...
				FBulletNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, ClientRecvData.SyncState, ClientRecvData.AuxState);
				...
				FBulletNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, ClientRecvData.ServerFrame, ClientRecvData.SimTimeMS, 0);
			}
		...
		for (auto& It : Instances)
		{
			...
			{
				typename TBulletInstanceFrameState<ModelDef>::FFrame& FromFrameData = Frames.Buffer[FromFrame];
				typename TBulletInstanceFrameState<ModelDef>::FFrame& ToFrameData = Frames.Buffer[ToFrame];
				...
				FBulletNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, Instance.SyncState, Instance.AuxState);
				...
				FBulletNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, InterpolatedTimeMS, 0);
			}
		}
	}
...
private:
	...
	struct FInstance
	{
		...
		// Last interpolated values. Stored here so that we can maintain FBulletNetworkPredictionStateView to them
	...
	static void CopyFrameData(int32 SourceFrame, int32 DestFrame, TBulletInstanceFrameState<ModelDef>& Frames, FInstance& Instance)
	{
		typename TBulletInstanceFrameState<ModelDef>::FFrame& SourceFrameData = Frames.Buffer[SourceFrame];
		typename TBulletInstanceFrameState<ModelDef>::FFrame& DestFrameData = Frames.Buffer[DestFrame];
...
// ------------------------------------------------------------------------------
class IBulletIndependentInterpolateService
{
public:

	virtual ~IBulletIndependentInterpolateService() = default;
	virtual void Reconcile(const FBulletVariableTickState* TickState) = 0;
	virtual void FinalizeFrame(float DeltaTimeSeconds, const FBulletVariableTickState* TickState) = 0;
};


template<typename InModelDef>
class TBulletIndependentInterpolateService : public IBulletIndependentInterpolateService
{
public:
	...
	TBulletIndependentInterpolateService(TBulletModelDataStore<ModelDef>* InDataStore)
		: DataStore(InDataStore) { }
	...
	void FinalizeFrame(float DeltaTimeSeconds, const FBulletVariableTickState* TickState) final override
	{
		...
		for (auto& It : Instances)
		{
			...
			if (FromFrame != ToFrame)
			{
				...
				FBulletNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, InterpolationTimeMS, 0);
				
				if (NetworkPredictionCVars::DrawInterpolation())
				{
					FBulletNetworkPredictionDriver<ModelDef>::DrawDebugOutline(InstanceData.Info.Driver, FColor::Green, 0.f);

					const int32 DeltaFromHead = Instance.LastWrittenFrame - FromFrame;

					FBulletNetworkPredictionDriver<ModelDef>::DrawDebugText3D(InstanceData.Info.Driver, *FString::Printf(TEXT("[%d-%d] %d. %.2f"), FromFrame, ToFrame, DeltaFromHead, PCT), FColor::Black);
				}

				if (NetworkPredictionCVars::PrintSyncInterpolation())
				{
					UE_LOG(LogBulletNetworkPrediction, Display, TEXT(""));
					UE_LOG(LogBulletNetworkPrediction, Display, TEXT("Interpolation %s. Frames: %d - %d. InterpolationTimeMS: %d. PCT: %.2f"), ModelDef::GetName(), FromFrame, ToFrame, InterpolationTimeMS, PCT);

					UE_LOG(LogBulletNetworkPrediction, Display, TEXT("From: %d"), FromTimeMS);
					FBulletNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(FromFrameData.SyncState);

					UE_LOG(LogBulletNetworkPrediction, Display, TEXT("To: %d"), ToTimeMS);
					FBulletNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(ToFromData.SyncState);

					UE_LOG(LogBulletNetworkPrediction, Display, TEXT("Interpolated: "));
					FBulletNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(Instance.SyncState);
			...
			else
			{
				...
				{
					UE_LOG(LogBulletNetworkPrediction, Display, TEXT("[Interpolate] Interpolation out of sync. We don't have valid frames for this time for this sim. Frame %d. TimeMS: %d. @ %d"), FromFrame, ActualTimeMS, InterpolationTimeMS);
				}

				typename FInstance::FFrame& FrameData = Instance.ClientRecvFrames[FromFrame];
				FBulletNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, FrameData.SyncState, FrameData.AuxState);

				InstanceData.Info.View->UpdateView(FromFrame, ActualTimeMS, nullptr, FrameData.SyncState, FrameData.AuxState);
				
				FBulletNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, ActualTimeMS, 0);

				if (NetworkPredictionCVars::DrawInterpolation())
				{
					FBulletNetworkPredictionDriver<ModelDef>::DrawDebugOutline(InstanceData.Info.Driver, FColor::Yellow, 0.f);
					FBulletNetworkPredictionDriver<ModelDef>::DrawDebugText3D(InstanceData.Info.Driver, *FString::Printf(TEXT("[%d] %d/%d (%d)"), FromFrame, ActualTimeMS, InterpolationTimeMS, (InterpolationTimeMS-ActualTimeMS)), FColor::Black);
				}
			}
		}
	}
...
private:
	...
	struct FInstance
	{
		...
		//
		// It would be nice if we could configure the NetRecv to use an array of TBulletClientRecvData.
		...
		// TBulletClientRecvData also stores more than we would need to here - the InputCmd and ServerFrame aren't
		...
		TBulletNetworkPredictionBuffer<FFrame> ClientRecvFrames;
	};
