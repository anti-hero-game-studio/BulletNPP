...
struct TBulletTickUtil
{
	...
	using FrameDataType = typename TBulletInstanceFrameState<ModelDef>::FFrame;
	...
	static typename TEnableIf<!std::is_same_v<SimulationType, void>>::Type DoTick(TInstanceData<ModelDef>& Instance, FrameDataType& InputFrameData, FrameDataType& OutputFrameData, const FBulletNetSimTimeStep& Step, const int32 CueTimeMS, EBulletSimulationTickContext TickContext)
	{
		...
		FBulletNetworkPredictionStateView* View = Instance.Info.View;
		...
		TBulletNetSimLazyWriterFunc<AuxType> LazyAux((void*)&OutputFrameData.AuxState);
...
public:
	...
	void Tick(const FBulletNetSimTimeStep& Step, const FBulletServiceTimeStep& ServiceStep) override
	{
		Tick_Internal<false>(Step, ServiceStep);
	}

	void TickResim(const FBulletNetSimTimeStep& Step, const FBulletServiceTimeStep& ServiceStep)
	{
...
protected:
	...
	void Tick_Internal(const FBulletNetSimTimeStep& Step, const FBulletServiceTimeStep& ServiceStep)
	{
		...
		{
			TInstanceData<ModelDef>& Instance = DataStore->Instances.GetByIndexChecked(It.Value.InstanceIdx);
			TBulletInstanceFrameState<ModelDef>& Frames = DataStore->Frames.GetByIndexChecked(It.Value.FrameBufferIdx);
	...
	template<bool bIsResim>
	EBulletSimulationTickContext GetTickContext(ENetRole NetRole)
	{
		if (bIsResim)
		{
			switch(NetRole)
			...
			case ENetRole::ROLE_AutonomousProxy:
				return EBulletSimulationTickContext::Resimulate;
				break;
		...
		else
		{
			...
			case ENetRole::ROLE_Authority:
				return EBulletSimulationTickContext::Authority;
				break;
			case ENetRole::ROLE_AutonomousProxy:
				return EBulletSimulationTickContext::Predict;
				break;
			case ENetRole::ROLE_SimulatedProxy:
				return EBulletSimulationTickContext::Predict; // Fixme: all sim proxies are forward predicted now. We need to look at net LOD here?
				break;
			}
		}
	...
	{
		int32 TraceID;
		int32 InstanceIdx; // idx into TBulletModelDataStore::Instances
		int32 FrameBufferIdx; // idx into TBulletModelDataStore::Frames
	};
	...
	void Tick(float DeltaTimeSeconds, const FBulletVariableTickState* VariableTickState) final override
	{
		...
		for (TConstSetBitIterator<> BitIt(InstanceBitArray); BitIt; ++BitIt)
		{
			...
			while (ServerRecvData.LastConsumedFrame < ServerRecvData.LastRecvFrame)
			{
				...
				typename TBulletServerRecvData_Independent<ModelDef>::FFrame& NextRecvData = ServerRecvData.InputBuffer[NextFrame];
				...
				// Tick
				{
					...
					FBulletNetSimTimeStep Step {InputCmdMS, ServerRecvData.TotalSimTimeMS, OutputFrame };
